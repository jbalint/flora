...
The sensor \texttt{sens2/4} is different from \texttt{sens1/4} in that it
is declared using a
...
Several things need to be noted regarding sensors:
%% 
\begin{itemize}
\item  A sensor directive is required for each sensor \emph{used}
  (called) in the given file.
\item For each sensor \emph{defined} (by rules) in a .flr file, either an appropriate
  sensor directive or a prolog directive is required like so:
  %% 
\index{prolog directive}
\begin{verbatim}
  :- prolog sens1/4.
...
\end{verbatim}
  %% 
\item Sensors are intended to be used for non-logical predicates such as
  ...
  represented by Prolog predicates). For instance,
%% 
\begin{verbatim}
   ?- ?X(?,?,?,?).
\end{verbatim}
