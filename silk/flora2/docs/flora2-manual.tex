...
\item  Sensors are \emph{used} and \emph{defined} in separate files.
  ...
  \texttt{defsensor} directive and they are defined via rules.
\item
  ...
  Accordingly,
  two different directives are provided.
...
In the first \texttt{defsensor} directive, the middle argument is the name of the
...
{\bf Unimplemented}: In the future, sensor definitions will have the same
number of arguments as sensor declarations. The extra arguments will be
...
\end{verbatim}
%% 
This directive is to be placed in the file that contains the rules defining
the sensor and the number of arguments must match the number used in the
sensor declaration (i.e., two in our case, not four).
