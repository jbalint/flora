...
%% 
\begin{itemize}
  \index{sensor directive}
  \index{directive!sensor}
\item  Sensors are \emph{declared} and \emph{defined} in separate files.
  Declaration is done via the
  \texttt{sensor} directive and they are defined via rules.
\item
  Sensors can be defined in \texttt{.P} files
  using the Prolog syntax (Type 1 sensors)  or in
  \texttt{.flr} files using the \FLORA syntax (Type 2 sensors). 
  Accordingly,
  two different \texttt{sensor} directives are provided.
  ...
  \texttt{.flr} file 
  will also contain the rules that define Type 2 sensors. 
\item 
...
\item  Sensors are \emph{used} in modules other than those where they
  are defined. To use a sensor, it has to be declared with the
  \texttt{usesensor} directive.
\item \FLORA also provides executable versions of the directives 
  \texttt{sensor} and \texttt{usesensor}.  
\end{itemize}
%% 


There are two forms of the \texttt{sensor} directive. The first is used for
Type 1 sensors (defined in \texttt{.P} files); the second is used for Type
2 sensors, which are defined using \FLORA syntax in \texttt{.flr} files.   
%% 
\begin{verbatim}
...
\end{verbatim}
%% 
The first argument shows a sensor invocation template.  The last argument
is a \emph{guard}: a Boolean expression consisting of \texttt{nonvar/1} and
...
In the first \texttt{sensor} directive, the middle argument is the name of the
Prolog module in which the Prolog sensor is defined. 
In our case, \texttt{sens1} is declared as a sensor defined by 
rules in the Prolog module \texttt{sensorfoo}. In that case, \FLORA
will expect those rules to be in a file named \texttt{sensorfoo.P}
and that file should be found somewhere on the \FLORA load path.

From the perspective of the \emph{user} of a sensor,
the above sensors are binary predicates. However, 
from the perspective of the \emph{developer} of a sensor, sensors have two
additional ``hidden'' arguments. These arguments 
...
file in which the sensor is used (note: the file where the sensor is
...
to define \texttt{sens1} and \texttt{sens2}, one usees
predicates of arity 4, not 2.  
...
The sensor \texttt{sens2} 
is declared using a
two-argument sensor-directive, so its definition is expected to be in a
\texttt{.flr} file and then loaded into a separate \FLORA module. As before, the actual predicate to be defined must
have 4 arguments:
%% 
\begin{verbatim}
...
\end{verbatim}
%%
The last line in the above definition
emphasizes the fact
that in a sensor definition for \texttt{sens2} the
sensor developer must be aware of the hidden arguments, so the recursive
call to \texttt{sens2}
must have 4 arguments.
However, when sensors are
used in \emph{other} \FLORA modules, they must be referred to without the
hidden file/line arguments. They also must be declared using the
...
\texttt{@\_prolog} specification: the
...
\end{verbatim}
%% 
This directive is to be placed in the file containing the rules defining
the sensor and the number of arguments must match the number used in the
sensor's rules (four in our case, not two).
