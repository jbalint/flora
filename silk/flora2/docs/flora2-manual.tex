...
%%
User-defined functions (abbr., UDF) is syntactic extension that permits the users to enjoy
certain aspects of functional programming in the framework of a logic-based
language.
...
\texttt{Expr} and Body can contain occurrences of other UDFs, but those
...
\end{verbatim}
%% 
In the rule body, the occurrences of foo(s1,...,sn) are rewritten as
follows:
%% 
\begin{verbatim}
    ... :- ..., pred(...,foo(s1,...,sn),...), ...
...
\end{verbatim}
%% 
Since \texttt{sens1/4}  is defined in an external Prolog module, it must be
referenced using the \texttt{@\_prolog(sensorfoo)} idiom.
...
\texttt{.flr} file. As noted before, all sensors are 
Prolog predicates, so \FLORA will declare \texttt{sens2/4} as as a
prolog as if the user issued an explicit directive
%% 
\index{prolog directive}
%% 
\begin{verbatim}
...
\end{verbatim}
%% 
The sensor declaration must appear in the
