...
\subsection{Non-Termination Analysis}

\index{non-termination}
\index{terminyzer}

There are two main non-termination causes in \FLORA, i.e., infinitely
many calls and infinitely many answers, and both of them are formed by
a set of recursive subgoals together with function symbols. 
Since non-termination is normally very hard to debug, \FLORA thus 
implements an automatic non-\emph{Termin}ation Anal\emph{yzer},
\texttt{terminyzer}, to help the user to locate and explain
non-terminating behaviors. 

When a non-termination is suspected, we suppose that the execution is
stopped after a time limit set by the user or after the evaluation 
starts producing queries or answers that exceed certain size limits.
...
calls are issued) that actually caused the non-termination.
Below, we describe the usage of \texttt{Terminyzer} and more
...
Section~\ref{sec-logforest} and {\tt SummaryFile} is the analysis
summary in \FLORA loadable format.  
The entries in the summary file locate and explain a non-termination
if it happens and they are as follows.
%% 
\begin{itemize}
...
\item \emph{Unfinished tabled calls}.
  ...
  derivation tree for $\tt S_2$ because of the rule whose id is
  {\tt RId} (i.e., $\tt S_2$ matches this rule's head and $\tt S_1$
  matches one of its body subgoals) and $\tt S_1$ is incomplete, the
  following two \FLORA molecules are recorded: 
%% 
\begin{verbatim}
...
%% 
  ...
  to $\tt S_1$. 
  Both ids are non-negative integers and they preserve the
  temporal orders of subgoal and call creation during evaluation.
  {\tt Type} records the type of this subgoal and call as {\tt unfinishedcall}.
...
\item \emph{Infinitely many subgoals}. 
  If the trace is for a non-termination caused by infinitely many
  subgoals, then these subgoals have to be generated by a set of
  recursive predicates together with function symbols.
  Furthermore, every cycle of calls to these recursive predicates
  produce a set of increasingly deeper subgoals.
  For each of the subgoals produced by the first cycle of recursive
  calls, a node of type {\tt loopcall} is recorded as follows:
%% 
\begin{verbatim}
   node[nodeid->NId,subgoal->S,module->M,ruleid->RId,type->loopcall].
\end{verbatim}
%% 
  where {\tt nodeid, module}, and {\tt ruleid} are the same as in the
  case of {\tt unfinishedcall}. 
  
\item \emph{Infinitely many answers}. 
  When the trace is for a non-terminating evaluation caused by
  infinitely many answers, these answers are produced by a set of
  recursive predicates together with function symbols. 
  These set of recursive unfinished subgoals and their relationships
  are also identified. 
  If a tabled subgoal $\tt S_1$ in module {\tt M} is called from a
  derivation tree for $\tt S_2$ because of the rule whose id is
  {\tt RId} and $\tt S_1$ is producing answers which will returned to
...
%% 
  ...
  and {\tt nodes} are the sequence of involved subgoal node ids.
  This sequence also keeps calling relationship among its subgoals. 
  ...
  The number of such loops are stored as
%% 
\begin{verbatim}
...
non-terminations, i.e., the set of recursive predicates and their
respective rule ids that are responsible for either infinitely many
subgoals or infinitely many answers. 
