...
The general rule is that an explicitly labeled rule is always defeasible
unless the label is a variable. Rules that are labeled with variables are
always strict.
...
\index{label\{...\} primitive}
%%
Sometimes it is useful to be able to query the rule base based on rule
labels and heads. With a little more typing, similar queries can be issued
using the {\tt clause\{...\}} primitive. However, 
\emph{unlike} the {\tt clause} primitive, 
{\tt label}  queries only defeasible rules. For instance, if the program
contains the rules
%% 
\begin{verbatim}
     :- use_argumentation_theory.
...
\end{verbatim}
%% 
The original argumentation theory is then avaolable in the module {\tt ogcl}.
...
at the same time. The term {\tt id} is the identifier of the
...
\item  {\tt status(?I,?H)[info->?Info]}.\\
  ...
  head {\tt ?H} in rules labeled {\tt ?I}. The information returned
...
{\tt notBeaterFor(ExclusionConstraintId)}.    
For instance,
%% 
\begin{verbatim}
