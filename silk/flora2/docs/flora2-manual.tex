...
%% 
\begin{verbatim}
   unfinished_subgoal(SubgoalId)[subgoal->S1,module->M,ruleid->RId].
   unfinished_call(CallId)[parent->S2,child->S1, ruleid->RId].
\end{verbatim}
%% 
  where {\tt SubgoalId} is a unique id generated for each
...
\item \emph{Non-termination due to infinite number of subgoals}. 
  ...
  a fact of the following form is recorded:
%% 
\begin{verbatim}
   loop_subgoal(SubgoalId)[subgoal->S,module->M,ruleid->RId].
\end{verbatim}
%% 
  where {\tt SubgoalId, module}, and {\tt ruleid} are as before.
  The number of involved subgoals is kept as
%% 
\begin{verbatim}
   number_of_subgoals(loop,NumOfSubgoals).
\end{verbatim}
%% 
...
%% 
\begin{verbatim}
   answerflow_subgoal(SubgoalId)[subgoal->S1,module->M,ruleid->RId].
   answerflow_call(CallId)[parent->S2,child->S1,ruleid->RId].
\end{verbatim}
%% 
  where {\tt SubgoalId} and {\tt CallId} are the respective subgoal id and
  call id generated for unfinished calls. 
  The number of {\tt answerflow} subgoals and calls are kept as
%% 
\begin{verbatim}
...
%% 
  ...
  Furthermore, this set of recursive subgoals form a loop which is
  reported as 
%% 
\begin{verbatim}
   loop[subgoals->SubgoalIds].
\end{verbatim}
%% 
  where {\tt SubgoalIds} is the sequence of subgoal ids involved
  in the loop. 
  It also indicates the call-relationship among the subgoals in
  the sequence. 
  For instance, {\tt loop[subgoals->[5,6,5]]} says
