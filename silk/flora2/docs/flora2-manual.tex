...
\end{alltt}
%%
one needs to notify the compiler only about \texttt{p(?,?)}, since other
...
above, \texttt{sens1/2} and \texttt{sens2/2} are declared as sensors: the
...
In our case, \texttt{sens1/2} is declared as a sensor defined by 
...
From the perspective of the \emph{user} of a sensor,
the above sensors are binary predicates. However, 
from the perspective of a \emph{developer} of a sensor, sensors have two
extra ``hidden'' arguments. These arguments 
are automatically prepended by the compiler to the list of the arguments
that the user specified in a \texttt{sensor} directive. 

In any actual call to a sensor, the first argument will be \emph{always}
bound (at compile time) to the
file in which the sensor is used (note: where it is
used, not where it is defined) and the second 
argument will be bound to the line number
on which the call to the sensor occurs.
This is useful in case the sensor needs to issue an error
message to enable the user to quickly locate the offending call. 
Thus, in the actual rules that define sensors,
the first two arguments  of the sensors must be
variables.
The remaining arguments are up to the developer of the sensor's rules.

The upshot of the above is that,
to define \texttt{sens1/2} and \texttt{sens2/2}, one has to use
predicates of arity 4.  
Here is an example of a definition for \texttt{sens1/2}. As we just
explained, the actual predicate to be defined here must be \texttt{sens1/4}
and its first two arguments are reserved for file names and line numbers.  
%% 
\begin{verbatim}
   :- export sens1/4.
   sens1(F,L,X,Y) :-
...
The sensor \texttt{sens2/2} is different from \texttt{sens1/2} in that it
is declared using a
two-argument sensor-directive, so its definition is expected to be in a
\texttt{.flr} file. As before, the actual predicate to be defined must
be \texttt{sens2/4}, however. 
%% 
\begin{verbatim}
   sens2(?F,?L,abc,cde) :- !, %writeln(details=?F+?L)@_io.
   sens2(?F,?L,?X,?Y) :-
...
\end{verbatim}
%%
The last line in the above definition is not intended to be meaningful. Its
point is to emphasize the fact
that in a sensor definition for \texttt{sens2} the
sensor developer must be aware of the hidden arguments, so \texttt{sens2/4}
must be used when calling \texttt{sens2} recursively. However, when sensors are
used in the rest of the program, they must be referred to without the
hidden arguments. For instance,
%% 
\begin{verbatim}
   ?- sens1(?X,?Y), ?X=123, ?Y=aaaaa, sens2(?X,?Y), ?Y=345, ?X=bbbbb.
\end{verbatim}
%% 
There is no need to add the \texttt{@\_prolog} specification, since the
compiler knows how to properly render sensor calls. 


The following points should be kept in mind when declaring, using, and
developing sensors:
%% 
\begin{itemize}
\item Sensors are declared by the \texttt{sensor} directive with the same
  number of arguments with which sensors are \emph{called} by user programs.
  However, when a sensor is \emph{defined} by a rule, two hidden arguments must be
  added. They are bound at runtime to the file name and line argument,
  respectively.
\item  Appropriate sensor directives are required in any file in which a
  sensor is \emph{defined} (by rules)
  or \emph{used} (called).
\item \FLORA files (.flr) that contain sensor definitions \emph{cannot be
  added} to a \FLORA module; they can only be \emph{loaded} into modules
  thus overriding normal \FLORA rules in that module.
  It is therefore a recommended practice to collect sensor definitions in
  \emph{separate}  files
  and load them into a separate designated \FLORA module.
  \\
  Note that files containing only sensor definitions can be loaded into \emph{one}
  module without overriding other sensor definitions (only identically
  names sensors would be overwritten). This behavior is unlike the normal
  behavior of \FLORA rules and therefore consolidating all sensors in one
  module is feasible (and recommended) even if sensor definitions are
  spread across several files.

\item 
  If the definition of a sensor is recursive (either in a .P or a .flr
  file) then a table directive might be required, e.g., 
  %% 
\index{table directive}
\begin{verbatim}
   :- table sens1/4.
\end{verbatim}
  %% 
Note that this table declaration is part of the definition of a sensor, so
the hidden arguments must be taken into account. This is why
\texttt{sens1} is declared here as a 4-ary predicate.
\item Sensors are intended to be used as non-logical predicates such as
  builtins or predicates that interact with the environment. As such,
  sensors are not visible to HiLog queries (and internally they are
  represented by Prolog predicates). For instance, neither
%% 
\begin{alltt}
   ?- ?X(?,?,?,?).
\textnormal{nor}
...
\end{alltt}
%% 
will bind \texttt{?X}  to \texttt{sens1} or \texttt{sens2}.
\item As mentioned, sensors are treated as Prolog predicates but the
  details are taken care of by the compiler. So, when
  used, sensors do not need the \texttt{@\_prolog}  specifier.
\end{itemize}
%% 
