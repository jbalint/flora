...
First, you need Microsoft's {\tt nmake}, which can be downloaded from 
...
provided you have a Microsoft C++ compiler), then you can type
%%
\begin{alltt}
...
Note that a free-of-charge version of the Microsoft C++ compiler, called
\emph{Visual C++ Express Edition}, can be downloaded free of charge from
\url{http://www.microsoft.com/express/vc/}. It is also necessary to install
Windows Platform SDK accessible from the above page, if it is not installed
automatically, together with the compiler.  Important: prior to
using this compiler
(e.g., before executing {\tt makeflora}) in an MS-DOS window, it is
important to locate the setup file, which is usually called {\tt
  vcvars32.bat} or {\tt vcvarsall.bat}, and execute it in that window.
This batch file contains commands to set up the necessary environment
variables without which compilation will fail. Unfortunately, this file is
located in different places for different versions of the compiler, so one
must use the search facility in order to find it.
...
contains variables), it is treated as a label given
to the rule by the user. Therefore, a
term should never be used as the first conjunct of the
meta-information part unless a separate rule label is also provided.
If no rule label is needed, then one can put a variable instead. This
signals the compiler to skip the label part. For instance,
%% 
\begin{verbatim}
 @{?, some[meta->info]} foo(?X) :- bar(?X).
\end{verbatim}
%% 
As we have just mentioned, a rule label can have variables. For instance,
%% 
\begin{verbatim}
...
Instead of a ``normal'' term, a rule label can be an anonymous oid
constant, {\tt \_\#}.  This is useful when the user wants the compiler to
generate a new unique rule label automatically. For instance,
%% 
\begin{verbatim}
   @{_#} foo(?X) :- bar(?X).
\end{verbatim}
%% 
Numbered anonymous oids, e.g., {\tt \_\#1} or {\tt
...
A rule instance, $\rho_1$ labeled $i_1$, \emph{opposes} another rule
instance, $\rho_2$ labeled $i_2$, if and only if
...
$i_1$, \emph{overrides} the rule instance $\rho_2$ labeled $i_2$ if and
...
labeled $i_1$ \emph{refutes} another rule instance, $\rho_2$ labeled $i_2$, 
if and only if these rules are in conflict and the former rule instance
overrides the latter. The rule instance $\rho_1$
labeled $i_1$ \emph{rebuts} $\rho_2$ labeled $i_2$ if and only if the two
...
rule instance, $\rho_1$ labeled $i_1$, is disqualified in two cases:
%% 
\begin{itemize}
\item If it transitively defeats itself, i.e., there is a sequence of
  labeled rule instances $(i_1,\rho_1)$, $(i_2,\rho_2)$, ...,
...
\subsection{Specifying Argumentation Theories to Use}
...
\end{alltt}
%% 
The first rule cancels all rule instances whose label unifies with
...
the rule {\tt c3}.   More interesting, however, is the reason for {\tt
  tt3(bb)} being true. Note that rule labeled with {\tt c4} cancels the
...
instance, {\tt \_cancel(L2)} cancels only the rule labeled {\tt L2}. If more
...
\end{verbatim}
%% 
The most interesting feature here is the label {\tt id2} parametrized with
variables and the rule labeled {\tt id3}, which uses that parametrized
label. Note also that here the cancellation rules are conditional.
The effect is that rule {\tt id3} cancels the instances 
of the cancellation rule labeled {\tt id2(printer,Bob)} and
...
must be \emph{purely logical}. This includes both the labeled rules of the
...
\item  Cut, the predicates {\tt ground/1}, {\tt var/1}, {\tt nonvar/1}, and
  ...
  %% 
\begin{verbatim}
    %check_state(?s) :- ground(?s)@_plg(basics), !.
...
%% 
\begin{itemize}
\item  {\tt status(?I,?H)[defeatedBy -> ?Reason]}.\\
  ...
  %% 
  \begin{itemize}
  \item {\tt refutedBy(ruleId,ruleHead)}: In this case, all the rule
    id/head pairs that refute the rule(s) represented by the {\tt ?I/?H}
    pair (i.e., whose label unifies with {\tt ?I} and head with {\tt ?H})
    will be returned.
  ...
  \item {\tt disqualified}: This is returned if the rule is
    disqualified. (Recall that in the defauls {\tt \_gcl} theory a rule is
    disqualified if it transitively refutes/rebuts itself or if it is
    canceled.)
    In this case, an auxiliary method, {\tt disqualifiedDueTo}, can provide
    additional information, as described next. 
  \end{itemize}
  %% 
\item  {\tt status(?I,?H)[disqualifiedDueTo->selfdefeat(?Defeater,?Defeated)]}.\\
...
\item  {\tt status(?I,?H)[disqualifiedDueTo->canceled]}.\\
  This literal is true if all the rules whose label unifies with
  {\tt ?I} and head with {\tt ?H} are
  canceled. 
\item  {\tt status(?I,?H)[candidate]}.\\
  This method tells whether at least one body of a rule represented by the
  {\tt ?I/?H} pair is satisfied in the intended model. Such satisfaction is
  a prerequisite for any conflict or for deriving the rule head as true.
  However, this is not a sufficient condition, as the rule might be
  defeated by another rule.
\item  {\tt status(?I,?H)[conflictsWith -> ?Conflict].}\\
  This method returns all Id/head pairs of the rules that are in conflict
  with the rules represented by the {\tt ?I/?H} pair. 
\item  {\tt status(?I,?H)[defeats -> ?Mode]}.\\
  If the {\tt ?I/?H} pair defeats (rebuts or refutes) any rule, this method
  will provide the relevant information. The result, denoted as {\tt
    ?Mode}, can have one of these two forms:
  %% 
  \begin{itemize}
  \item  {\tt refutes(?Refuted)}: Here {\tt ?Refuted} gets bound to Id/head
    pairs of the rules refuted by {\tt ?I/?H}. If none exists, the query
    fails.
  \item  {\tt rebuts(?Rebutted)}: Here {\tt ?Rebutted} gets bound to Id/head
    pairs of the rules refuted by {\tt ?I/?H}. If none exists, the query
    fails.
  \end{itemize}
  %% 
\end{itemize}
%% 
All these methods are provided by the {\tt \_gcl} module, and thus must be
called accordingly. For instance, 
%% 
\begin{verbatim}
    ?- status(?I, configuration(0,block4,square7))[conflictsWith->?X]@_gcl.
    ?- status(?I,configuration(nxt(nxt(0)),block4,square3))[defeatedBy->?X]@_gcl.
\end{verbatim}
%% 
