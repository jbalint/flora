...
provided you have a Microsoft C++ compiler), then you can type\footnote{
  ...
  or \FLORA) can be downloaded free of charge from \url{http://www.microsoft.com/express/vc/} It is
...
The basic mechanisms by which the user can control the course of defeasible
...
defeasible theory} decides which ground instances of which rules are
...
predicates, as defined in Section~\ref{sec-overriding-conflict}, but this
...
\end{itemize}
%% 
Different defeasible theories may assign different meaning to these
...
Rule disqualification is not so commonly agreed upon notion as the other
two.  Some defeasible theories do not use it at all (i.e., nothing is
disqualified).  The default defeasible theory in \FLORA postulates that a
rule instance, $\rho_1$ labeled $i_1$, is disqualified if it transitively
defeats itself, i.e., there is a sequence of labeled rule instances
$(i_1,\rho_1)$, $(i_2,\rho_2)$, ..., $(i_{n-1},\rho_{n-1})$, $(i_n,\rho_n)$,
such that $i_n=i_1$, $\rho_n=\rho_1$, and at each step
$(i_i,\rho_i)$ either refutes or rebuts $(i_{i+1},\rho_{i+1})$.

These notions are very useful for the purpose of debugging (finding out why
...
\subsection{Specifying Defeasible Theories to Use}
...
\index{use\_defeasible\_theory directive}
%% 
\begin{alltt}
...
that have {\tt true} as their bodies.
...
\item  Cut, the predicates {\tt ground/1}, {\tt var/1}, {\tt nonvar/1}, and
  similar.
  This predicates can be used only if the alternatives (when
...
\end{verbatim}
  %% 
  can be used in the body of a defeasible rule (or of the rules defining
  {\tt \_overrides} and {\tt \_opposes}) because the alternative to being
  grounded will abort the inference process.
...
To help the user debug defeasible knowledge base, the default defeasible
...
\item  {\tt status(?I,?H)[defeatedBy -> ?Reason]}.\\
  ...
  \item {\tt disqualified}: This is returned if the rule is
    ...
    disqualified if it transitively refutes/rebuts itself.)
    In this case, an auxiliary method, {\tt disqualifiedDueTo}, can provide
    additional information, as described next. 
  \end{itemize}
  %% 
\item  {\tt status(?I,?H)[disqualifiedDueTo->defeats(?Defeater,?Defeated)]}.\\
  ...
  these pairs one can discover a cycle of defeats starting with {\tt ?I/?H}
  and understand why the corresponding rules were disqualified.
...
Since defeasible reasoning theories are implemented using tabled
...
To simplify this task, the \FLORA modules that implement defeasible
...
\end{verbatim}
%% 
where {\tt foo} is the name of the defeasible theory in question. For
instance, for the GCL defeasible theory, this task can be performed via the
query
%% 
