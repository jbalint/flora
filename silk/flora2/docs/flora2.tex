...
%%
\FLORA supports two kinds of negation: the usual Prolog \emph{negation as
  failure} \cite{Cla78} and \emph{default negation} based on
\emph{well-founded semantics}
\cite{gelder-alternating-89,gelder-ross-schlipf-91}.
...
Explain what cannot be negated ...
...
semantic options by examining every loaded module via the {\tt
  \_isloaded/1} builtin and then posing the desired {\tt
  semantics\{...\}} query.
...
The following aggregate operators are supported in \FLORA: {\tt min}, {\tt
  ...
  collectbag}. The aggregate function {\tt set} is synonmous with
...
\index{labeling construct}
\index{rule label}
\index{rule meta-data}
\FLORA supports the use of the \emph{labeling construct} of the form
\@\{\emph{Label},\emph{Meta}\} in front of any term of formula (e.g., rule,
...
In all other cases the labeling construct is discarded.

The \emph{Label} part of the labeling construct
must be a constant, while \emph{Meta} is a comma-separated list of literals.
Both the \emph{Label} and \emph{Meta} are optional. If the first conjunct
...
Instead of a constant, the rule label can be an anonymous oid constant,
...
\end{verbatim}
%% 
The numbered oid feature also allows to ``capture'' the newly generated
rule label in the program. For instance,
%% 
\begin{verbatim}
...
In contrast, rule labels are provided mostly to support \emph{defeasible
  reasoning}---see Section~\ref{sec-defeasible}.   Rule labels are
\emph{not} required to be unique, and in many cases they are not. In addition,
labels cannot be assigned to rules that have transactional literals (marked
with the \%-sign) in the rule head: defeasible reasoning with
side-effectful rules is not supported.
...
\begin{alltt}
  ...
  _overrides({\em{}RuleLab1},{\em{}RuleHead1},{\em{}RuleLab2},\emph{RuleHead2}).
\end{alltt}
%% 
The first form of the {\tt \_overrides} statement says that the rule with
label \emph{RuleLab1} overrides the rule with the rule label \emph{RuleLab2}
regardless of what the heads of those rules are.
%% 
\begin{alltt}
...
formulas \emph{AtomForm1} and \emph{AtomForm2} oppose each other
whenever they occur in the rule heads---regardless of the labels of those
rules.
