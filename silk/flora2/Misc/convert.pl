...
if ($ARGV[0] eq "-var_trans" or $ARGV[0] eq "-single_multi" or $ARGV[0] eq "-module_name" or $ARGV[0] eq "-all") { $signal = $ARGV[0];  }
  else { die "Usage:\n   perl convert.pl -var_trans|-single_multi|-module_name|-all  DIRECTORY\n"; }
...
sub parse_file {
    ...
    if ($cd[@cd - 1] eq "flr" 
       ...
       if ($signal eq "-single_multi" or $signal eq "-all")
	 {
       ...
       if ($signal eq "-module_name" or $signal eq "-all")
	 {
	   $lines = &flora_module_name($lines);
	 }
...
# ------------------------------------------------------------------------------
# --- change @flora(foo) to @_foo.
# --- doesn't work when ')' appears in (), such as @flora(a/*)*/)
# ------------------------------------------------------------------------------
sub flora_module_name {
    my ($string) = @_;

    my $before, my $after, my $module_name;
    my $a1, my $a2, my $b;
    my $after_processed;
    ...
    if ($string =~ /($a1)/){
      ...
      $after_processed = &flora_module_name($after);

      $string = $before.$b.$module_name.$after_processed;
