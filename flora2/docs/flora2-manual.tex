...
\begin{verbatim}
     ...
     ${\if foo\then bar\else moo} =..
                    [control(ifthenelse),main,${foo@main},${bar@main},${moo@main}]
     ${\while foo \do bar} =..[control(whiledo),main,${foo@main},${bar@main}]
     ${insert{?V|p(?V)}} =.. [update(insert),[?V],${p(?V)@main}]
     ${deleteall{?V|p(?V)}} =.. [update(deleteall),[?A],${p(?A)@main}]
     ${?X=min{?V|p(?V)}} =.. [logic(and),${?T=min{?V|p(?V)@main}},?X=?T]
     avg{?V[?T]|p(?V,?T)} =.. [aggregate(avg),?V,[?T],${p(?V,?T)@main},?Result]
     count{?V[?G]|p(?V,?G)} =.. [aggregate(count),?V,[?G],${p(?C,?G)@main},?R]
\end{verbatim}
%% 
...
not be a valid \FLSYSTEM term or formula.
Most notably, the update operators, such as \texttt{insert\{...\}} and
\texttt{delete\{...\}}, cannot yet be reconstructed by putting, say,
\texttt{[update(insert),[?V],\${cond(?V)@main}]} on the right side of
\texttt{=..}.
...
\section{Displaying \FLSYSTEM Terms and Goals}\label{sec-flrshow}
