...
of \emph{atomic formulas} with the help of connectives, such as
conjunction, disjunction, the various negations, and more. The main forms
of atomic formulas are
%% 
\begin{itemize}
\item  F-logic \textbf{molecules} (or \textbf{frames}) are used for
...
\end{itemize}
%% 
The different types of molecules and predicates are described in the
respective sections.
The main components used to construct predicates and molecules include:
%% 
\begin{itemize}
\item  \textbf{Variables}, which are statements of the form \emph{?Varname}.
...
\item \textbf{Quasi-constants} and
  \textbf{quasi-variables}. Quasi-constants are symbols that get substituted
  ...
  As example of a quasi-variable is \emph{\bs{}?C}. This variable can
  appear in the body of a rule and it gets substituted
  with the name of the module that invoked that rule. This is
...
\item \textbf{Auxiliary symbols}, such as \texttt{->}, \texttt{!},
  ...
  the aforesaid components to form atomic formulas.
\end{itemize}
%% 
...
Typed variables can be bound to classes, which will restrict them so that
they will be unifiable only with members of the specified classes. This
includes the classes associated with all the primitive data types such as 
\texttt{\bs{}integer}, \texttt{\bs{}real}, \texttt{\bs{}time},
\texttt{\bs{}list}, \texttt{\bs{}duration}, etc., which are discussed in
Section~\ref{sec-data-types}. 
...
class. Theoretically this is equivalent to making a test like
\texttt{?X:Class} but in practice typed variables can be much more efficient.
Testing \texttt{?X:Class} means enumerating the entire class
\texttt{Class}, which can be expensive when the class is large and may not
...
\begin{verbatim}
 ...
 ${wish(nonvar(?X))^p(?X)} =.. [quantifier(delay),wish,${nonvar(?X)@\plg},${p(?X)}].
\end{verbatim}
%% 
...
arbitrary classes, not just data types; it has already been deiscussed in
...
\texttt{\bs{}btp}). They are described here by their
signatures.
%%
...
\item      {\tt ?- \bs{}"http://foo.bar.com/abc"$\hat{~}\hat{~}$\bs{}iri[host -> 'foo.bar.com']@\bs{}btp}  
\end{itemize}
...
\subsection{ The Primitive Types $\bs{}$decimal, $\bs{}$integer, and
  $\bs{}$short}
...
{\tt \bs{}double}, while \texttt{\bs{}short} and  {\tt \bs{}integer} for {\tt \bs{}long}.   
...
%%
\begin{verbatim}
  :- compiler_options{production_mode,prolog(spec_repr)}.
\end{verbatim}
%%
...
\paragraph{Standard I/O interface.}
...
Instead, there is a notion of standard input and output ports, which are
usually associated with and interactive window, and user-defined ports.
Standard input and output ports always exist, while user-defined ports
are created and destroyed when files are open and closed. This is somewhat 
similar to stream-based I/O, but nothing gets pushed on stacks. Instead,
all read and write operations must use the appropriate ports explicitly.
%%
\begin{itemize}
\item  {\tt ?Filename[open(?Mode,?Stream)]} --- opens {\tt ?Filename} with mode
  {\tt ?Mode} (which can be {\tt r}, {\tt w}, or {\tt a}) and binds {\tt
  ?Stream} to the file handle.
  The file can live remotely at some URL. In that case, {\tt ?Filename}
  must be instantiated to the form {\tt url(...)}.  

  One way to think of 
  \texttt{?Filename[open(r,?Stream)]} is that it is
  like \texttt{see(?Filename)}
  followed by \texttt{seeing(?Stream)}. Similarly,
  \texttt{?Filename[open(w,?Stream)]} 
  is like \texttt{tell(?Filename)} followed by \texttt{telling(?Stream)}.
  But \texttt{open} provides one more option, \texttt{a}, which does not
  exist in stream-based I/O.  
\item {\tt ?Stream[close]} --- closes {\tt ?Stream}. It is similar to
  \texttt{told} but it can close any stream, not just the default one. 
\item {\tt ?Stream[read(?Result)]} --- bind {\tt ?Result} to the next term
  in the previously open input {\tt ?Stream}.
\item {\tt ?Stream[write(?Result)]} --- write {\tt ?Result} out to the
  previously open output {\tt ?Stream}.
\item {\tt ?Stream[fmt\_write(?Format,?O)]} --- same, but use {\tt ?Stream} for the
  output. 
\item {\tt ?Stream[fmt\_read(?Format,?Result,?Status)]}  ---  same, but use {\tt
    ?Stream} for input.
\item {\tt ?Stream[write\_canonical(?Term)]} --- same, but use {\tt ?Stream} for
  output.
\item {\tt ?Stream[read\_canonical(?Term)]} --- same, but use {\tt ?Stream} for
  input.
\item {\tt ?Stream[readline(?Type,?String)]} --- same, but use {\tt
    ?Stream} for input.
\end{itemize}
%%
...
\texttt{foo.txt} has \texttt{foo. p(a).} then the following will result:
%% 
\begin{verbatim}
   ?- (see('foo.txt'), read(?X), read(?Y), seen)@\io.
...
stream is now associated with that file, but we still have a reference
...
stream and \texttt{told} closes that stream. The last command closes
\texttt{?Stream1} explicitly. 
...
The above output methods and predicates make all effort to present the
...
can be seen as an intermediate device. There are two display modes:
\texttt{default} and \texttt{debug}.  At present, the only difference is
that in the \texttt{debug} mode
the Skolems are written out using their internal representation. Other
similar features might be added to that mode
later. For instance,
%% 
\begin{verbatim}
  ? setdisplaymode{debug}.
  ?- insert{p(\#)}, p(?_X), writeln(?_X)@\io.
  _$_$_ergo'newoid2|_2
