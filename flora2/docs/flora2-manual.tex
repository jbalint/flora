...
\FLSYSTEM is a sophisticated object-based  knowledge representation and
...
\index{FLORID}
%%
The programming language supported by \FLSYSTEM is a dialect of \fl with
...
\FLSYSTEM is available
on \FLSYSTEM's Web site at \url{http://flora.sourceforge.net}
}
{\input{./hooks/intro}}
...
development version of the software directly from the SVN repository.\footnote{
  \url{http://sourceforge.net/p/flora/src/HEAD/tree/trunk/flora2/}
}
%% 
...
The advantage of this procedure is that it does not require any special
software, such as the C++ compiler or make/nmake to be installed. Not requiring
this extra software is especially advantageous to Windows users and, to
some degree, also Mac users.
...
\paragraph{Unix based systems (Linux, Mac, Cygwin, etc.).}
The Unix-style download is the file \texttt{flora2bundle.tar.gz}. 
Unpack it into a suitable installation directory, \textbf{DIR}:
%% 
\begin{quote}
  \tt
...
\end{quote}
%% 
This will create two subfolders: \texttt{flora2} and \texttt{XSB}.

\noindent
Next run the following command from within \textbf{DIR}: 
%% 
\begin{quote}
  \tt
   ./flora2/bundle\_config.sh
\end{quote}
%% 
This will install both XSB and \FLSYSTEM, appropriately configured.
The script to run \FLSYSTEM is \texttt{\textbf{DIR}/\ENGINENAME/\ENGINERUN}.  
...
Once Visual C++ Express is installed and \FLSYSTEM is checked out from SVN,
use the following commands to configure
the system (assuming that XSB is already installed and configured).
First, go to the Start menu then Control Panel then System (depending on your
...
\begin{quote}
  \tt
 ;C:{\bs}Program Files{\bs}Microsoft Visual Studio 10.0{\bs}VC{\bs}BIN
\end{quote}
%% 
Save the changes. 
...
\begin{quote}
  \tt
   ...
   makeflora clean\\
   \tt
   makeflora \textnormal{\emph{path-to-}}{\bs}XSB{\bs}bin{\bs}xsb.bat
\end{quote}
%%
Here \texttt{flora2}  is the root directory 
for \FLSYSTEM in your SVN checkout; it should have the form
\emph{something}$\backslash${\tt flora2}. The argument
\textnormal{\emph{path-for-}}\texttt{{\bs}XSB{\bs}bin{\bs}xsb.bat}
must be the full path name for the XSB invocation script.
Note, that unlike Unix, there should be no ``all'' keyword
in the Windows version of the \texttt{makeflora} command.

\noindent
\textbf{64-bit version:}  
If you compiled XSB as a 64-bit application, use \texttt{xsb64.bat} above instead of
\texttt{xsb.bat}.  

It may be useful to set the environment variable \texttt{HOME}
on your Windows
system to some directory accessible from your account. XSB and \FLSYSTEM
will place
various auxiliary files in the subfolder \texttt{.xsb}  in that
directory.
If \texttt{HOME}  is
not set,
XSB will use the location specified in the \texttt{USERPROFILE}
system variable.
...
\begin{quote}
\tt
   makeflora -c \textnormal{\emph{path-for-}}{\bs}XSB{\bs}bin{\bs}xsb.bat
\end{quote}
%%
...
run. However, this latter step was not necessary in Visual
C++ Express 10.0.
...
%%
\begin{alltt}
 alias \ENGINENAME='~/ENGINEDIR/\ENGINENAME/\ENGINERUN'
\end{alltt}
%%
...
\noindent
\FLSYSTEM comes with a number of demo programs that live in
%%
\begin{alltt}
 .../\ENGINENAME/demos/
\end{alltt}
...
``\verb|\demo(demo-filename).|''
at the \FLSYSTEM prompt, {\it e.g.},
%%
\begin{alltt}
 \prompt \bs{}demo(flogic_basics).
\end{alltt}
%%
There is no need to change to the demo directory, as {\tt \bs{}demo} knows
...
\texttt{.\flrext} suffix is not required)  and that file contains
%% 
\begin{verbatim}
   writeln(Welcome!)@\plg.
   insert{foo(bar)}.
...
%%
\begin{alltt}
  \prompt \bs{}load(program).
  \prompt \bs{}load(url(program)).
\end{alltt}
%%
...
%% 
\begin{alltt}
  \prompt \bs{}load(url('http://example.com/test1')).
...
There is an important special case of the {\tt \bs{}load} and {\tt [...]}
command when the file name is missing. In that case,
...
\begin{alltt}
  \prompt [>{}>module].
  \prompt \bs{}load(>{}>module).
\end{alltt}
%%

\paragraph{Adding rule-bases to modules.}
When the {\tt \bs{}load} command loads a rule base into a module, it first wipes
...
a module. This operation, called {\tt \bs{}add}, does not erase the old
...
\FLSYSTEM's \texttt{\bs{}load}  and \texttt{\bs{}add} commands try to be smart in
...
By default, \FLSYSTEM prints out all answers. If only one at a time is
desired, type in the following command: {\tt \_one}. You can revert back to
the all-answers mode by typing {\tt \_all}. Note: {\tt \bs{}one} and {\tt \bs{}all}
...
\end{alltt}
%%
the {\tt \bs{}one} directive will affect {\tt goallist2},
but \emph{not} {\tt goallist1}. This is because {\tt goallist1} executes in
the same query as {\tt \bs{}one} and thus is not affected by this directive.
...
\item {\tt \bs{}end}:
    Show the help info.
\item {\tt \bs{}compile(file)}:
    Compile FILE.\flrext for the default module {\tt main}.
\item {\tt \bs{}compile(file>{}>module)}:
    Compile FILE.\flrext for the module {\tt module}.
  \item {\tt \bs{}load(file>{}>module)}: Load {\tt file.\flrext} into the module {\tt
      module}. If you specify {\tt file.P} or {\tt file.\ofile} then will load
    these files.
  \item {\tt \bs{}load(file)}: Load {\tt file.\flrext} into the default module {\tt
      main}. If you specify {\tt file.P} or {\tt file.\ofile} then will load
    these files.
\item {\tt \bs{}compile(file)}:
    Compile FILE.\flrext for \emph{adding} to the default module {\tt main}.
\item {\tt \bs{}compileadd(file>{}>module)}:
    Compile FILE.\flrext for adding to the module {\tt module}.
  \item {\tt \bs{}add(file>{}>module)}: Add {\tt file.\flrext} to the module {\tt
      module}.
  \item {\tt \bs{}add(file)}: Add {\tt file.\flrext} to the default module {\tt
      main}. 
...
\item {\tt \bs{}warnings(type)}:
  ...
  dependency checker warnings. Note that {\tt \bs{}warnings(compiler)}
  turns off dependency checking completely.
\item {\tt \bs{}end}:
    Say Ciao to \FLSYSTEM, stay in Prolog.
\item {\tt \bs{}halt}:
    Quit both \FLSYSTEM and Prolog.
\end{itemize}
%%
Of course, many other executable directives and queries can be executed at
the \FLSYSTEM shell. These are described further in this manual

In general, \FLSYSTEM built-in predicates whose name is of the form {\tt
  fl[A-Z]...} are either the \FLSYSTEM shell commands or predicates that can
be used in Prolog to control the execution of \FLSYSTEM modules. We will
...
command {\tt \bs{}demo(FILE)} first looks for {\tt FILE} in the \FLSYSTEM demo
...
All Prolog commands can be executed from \FLSYSTEM shell, if the corresponding
...
introductory examples illustrating the use of \fl and \FLSYSTEM. Meanwhile, the
reader can read the \FLSYSTEM tutorial
available on the \FLSYSTEM Web site: 
\url{http://flora.sourceforge.net/tutorial.php}. 
}
{\input{./hooks/byexample}}
...
\item In \cite{KLW95}, types are always inheritable, but values are not.
  In \FLSYSTEM, information abount an object
...
\item \FLSYSTEM also has many extensions compared to F-logic. First, it
  ...
  notation, etc.
\ifundef{\isfloraman}{
\item Proprietary \FLSYSTEM-specific extensions include arbitrary
  first-order formulas in the rule head, Unicode support, scalability
  improvements, and OWL connectivity.
}{}
\end{enumerate}
%%
...
\begin{example}
  ...
  a fragment of a \FLSYSTEM program that represents a database of scientific
  publications.
\end{example}
...
e.g., \texttt{'JOHN SMITH'},\texttt{'default.\flrext'}.  Internally, \FLSYSTEM
...
%
Like Prolog character lists, \FLSYSTEM character lists (\emph{charlists}) are
enclosed in a pair of double quotes (\texttt{\dq}).  
For instance,
\mbox{\texttt{[102,111,111]}} is the same as \texttt{{\dq}foo{\dq}}.

Escape characters are recognized inside \FLSYSTEM charlists similarly to
\FLSYSTEM symbols.  However, inside a charlist, a single quote character does
...
expression, \FLSYSTEM provides another operator, {\tt \bs{}is}.  For example, {\tt
  ?X \bs{}is 3+4} will bind {\tt ?X} to the value {\tt 7}.
...
1200        & \RULELOGNAF & well-founded negation & not applicable & unary \\ \cline{2-5}
            & \bs{}neg & explicit negation & not applicable & unary \\ \cline{2-5}
            & $\backslash+$ & Prolog-style negation & not applicable & unary \\ \hline
...
In some cases the developer might require the program to retain the
information about the source code where the various \FLSYSTEM statements
occur. To this end, the compiler provides a number of
\emph{quasi-constants}, which get substituted for real constants at compile
or a loading time. The supported constants are:
%% 
\begin{itemize}
  \index{quasi-constant}
...
\item  \texttt{\bs{}@} -- the module into which the file containing this
  ...
  to be the atom \texttt{main}. 
\item \texttt{\bs{}@F} -- the file in which this quasi-constant occurs. In the
  \FLSYSTEM shell, this quasi-constant is set to be the atom
  \texttt{'(runtime)'}. 
...
that \FLSYSTEM replaces them by constants. However, this happens at \emph{run
  time}, during the program evaluation. The supported quasi-variables are:
%% 
\begin{itemize}
  \index{quasi-variable}
...
quasi-constants \verb|\#| and \verb|\#1|, \verb|\#2|, etc.,
...
\paragraph{Co-reference of Skolem symbols.}
%%
...
compiler to generate a new Skolem symbol. To solve this problem, \FLSYSTEM allows 
\emph{numbered Skolem symbols}, which are of the form \verb|\#132|, {\it
  i.e.}, \verb|\#| with a number attached to it. For instance, 
%%
\begin{quote}
\begin{verbatim}
\#1[ssn->123, father->f(\#1)[name->John, spouse->\#[name->Mary]]].
\#1[self->\#1].
\end{verbatim}
\end{quote}
%%
The first time the compiler finds \verb|\#1| in the first clause above,
it will generate a new oid. However, the second occurrence of \verb|\#1|
in the \emph{same} clause (i.e., \verb|f(\#1)|)  will use the oid
previously generated for the first occurrence. On the other hand,
occurrences of \verb|\#1| in \emph{different} clauses are substituted with
different oids. Thus, the occurrences of \verb|\#1| in the first and second
clauses above refer to different objects.

The same numbered Skolem can be co-referenced in \emph{different} facts and
...
middle statements are facts. The reason why \texttt{\bs{}\#1} is because all
...
{\tt newoid\{...\}} (or, equivalently, {\tt skolem\{...\}}) built-in. For instance, 
%%
\begin{alltt}
\prompt newoid\{?X\}.
 
?X = \FLPREFIX'dyn_newoid308   (\#)
...
\begin{verbatim}
  ...
  test1 :- abc[prop1->\#, prop2->\#5, prop3->\#5].
...
\end{verbatim}
%% 
Here \texttt{\bs{}\#1} inside \texttt{p(...)}  is a constant, while in 
inside \texttt{q(...)} it is a check that the argument of \texttt{q/1} is
bound to a Skolem term.  
...
\end{verbatim}
%% 
will succeed. Of course, {\tt ?- newoid{?X}, ?X:\bs{}skolem}   
will also succeed, but the {\tt \bs{}skolem} class contains all Skolem terms,
while the {\tt \bs{}isskolem/1} predicate is true of Skolem constants only.

Finally, we note that both the {\tt \bs{}isskolem/1} predicate builtin and the {\tt
...
\texttt{\bs{}isskolem/1} meta-predicate, which can tell whether a particular
...
\begin{itemize}
  \index{\bs{}isnumber/1}
\item \texttt{\bs{}isnumber/1} --- tests whether the argument is (or is bound
  to) a number.
  \index{\bs{}isinteger/1}
\item \texttt{\bs{}isinteger/1} --- tests whether the argument is (or is bound
  to) an integer number. 
  \index{\bs{}isfloat/1}
\item \texttt{\bs{}isfloat/1} --- tests whether the argument is (or is bound
  to) a floating point number.
  \index{\bs{}isdecimal/1}
\item \texttt{\bs{}isdecimal/1} --- tests if the argument is (or is bound
  to) a decimal number. At present, this is the same as \texttt{\bs{}isnumber/1}.
  \index{\bs{}isatom/1}
\item \texttt{\bs{}isatom/1} --- tests whether the argument is (or is bound
  to) a Prolog atom.
  \index{\bs{}iscompound/1}
\item \texttt{\bs{}iscompound/1} --- tests whether the argument is (or is bound
  to) a compound term, i.e., a term that has a non-zero-ary function
  symbol.
  \index{\bs{}isatomic/1}
\item \texttt{\bs{}isatomic/1} --- tests if the argument is (or is bound
  to) a Prolog atom or a number.
  \index{\bs{}islist/1}
\item \texttt{\bs{}islist/1} --- tests if the argument is (or is bound
  to) a list term.
  \index{\bs{}ischarlist/1}
\item \texttt{\bs{}ischarlist/1} --- tests whether the argument is (or is bound
  to) a list or ASCII characters.
  \index{\bs{}isiri/1}
\item \texttt{\bs{}isiri/1} --- tests whether the argument is (or is bound
  to) an IRI data type.
  \index{\bs{}isstring/1}
\item \texttt{\bs{}isstring/1} --- tests whether the argument is (or is bound
  to) a string data type.
  \index{\bs{}issymbol/1}
\item \texttt{\bs{}issymbol/1} --- tests whether the argument is (or is bound
  ...
  \index{\bs{}isvar/1}
\item \texttt{\bs{}isvar/1} --- tests if the argument is an unbound variable.
  \index{\bs{}isnonvar/1}
\item \texttt{\bs{}isnonvar/1} --- tests if the argument is not an unbound variable.
  \index{\bs{}isground/1}
\item \texttt{\bs{}isground/1} --- tests if the argument is a ground term (or
  is bound to one).
  \index{\bs{}isnonground/1}
\item \texttt{\bs{}isnonground/1} --- tests if the argument is not a ground term.
\end{itemize}
%% 

\index{delayable builtin}
\index{builtin!delayable}
In addition, some of the above builtins have \emph{delayable} 2-argument
...
computation, then the outcome depends on the second argument: if it is
\texttt{must} then an error is issued; if it is \texttt{wish} then the
builtin is quietly evaluated to \emph{false}.  For instance,
%% 
\begin{alltt}
\prompt \bs{}isinteger(?X).

No

\prompt \bs{}isinteger(?X), ?X=1.

No

\prompt \bs{}isinteger(?X,wish), ?X=1.

?X = 1

Yes

\prompt \bs{}isinteger(?X,must), ?X=1.

?X = 1

Yes

\prompt \bs{}isinteger(?X,must).

++Abort[\errorsystem]> in file (runtime) on line 1: instantiation error in builtin:
                \bs{}isinteger(_h3464); unbound argument

\prompt \bs{}isinteger(?X,wish).

No
\end{alltt}
%% 
Thus, the delayable versions of the above buitins are insensitive to the
...
variable,
since \texttt{\bs{}isinteger/2} will be delayed past the moment
\texttt{foo(?X)} is evaluated. Using \texttt{\bs{}isinteger(?X)} instead will
do the job.

The delayable versions of the aforesaid builtins are listed below. The
delayable versions of
\texttt{\bs{}isvar/2}, \texttt{\bs{}isnonvar/2}, \texttt{\bs{}isground/2}, and
\texttt{\bs{}isnonground/2} have slightly different semantics.
%% 
\begin{itemize}
  \index{\bs{}isnumber/2}
\item \texttt{\bs{}isnumber/2} --- the delayable version of \texttt{\bs{}isnumber/1}.
  \index{\bs{}isinteger/2}
\item \texttt{\bs{}isinteger/2} --- the delayable version of \texttt{\bs{}isinteger/1}.
  \index{\bs{}isfloat/2}
\item \texttt{\bs{}isfloat/2} --- the delayable version of \texttt{\bs{}isfloat/1}.
  \index{\bs{}isdecimal/2}
\item \texttt{\bs{}isdecimal/2} --- the delayable version of \texttt{\bs{}isdecimal/1}.
  \index{\bs{}isatom/2}
\item \texttt{\bs{}isatom/2} --- the delayable version of \texttt{\bs{}isatom/1}.
  \index{\bs{}iscompound/2}
\item \texttt{\bs{}iscompound/2} --- the delayable version of \texttt{\bs{}iscompound/1}.
  \index{\bs{}isatomic/2}
\item \texttt{\bs{}isatomic/2} --- the delayable version of \texttt{\bs{}isatomic/1}.
  \index{\bs{}islist/2}
\item \texttt{\bs{}islist/2} --- the delayable version of \texttt{\bs{}islist/1}.
  \index{\bs{}ischarlist/2}
\item \texttt{\bs{}ischarlist/2} --- the delayable version of \texttt{\bs{}ischarlist/1}.
  \index{\bs{}isiri/2}
\item \texttt{\bs{}isiri/2} --- the delayable version of \texttt{\bs{}isiri/1}.
  \index{\bs{}isstring/2}
\item \texttt{\bs{}isstring/2} --- the delayable version of \texttt{\bs{}isstring/1}.
  \index{\bs{}issymbol/2}
\item \texttt{\bs{}issymbol/2} --- the delayable version of \texttt{\bs{}issymbol/1}.
  \index{\bs{}isvar/2}
\item \texttt{\bs{}isvar/2} --- the delayable version of \texttt{\bs{}isvar/1}.
  If the argument \texttt{?X} in \texttt{\bs{}isvar(?X,must)} \emph{at the end of
  the query computation} is \emph{not} an unbound
  variable, an error is issued. Otherwise, the result is true.
  For \texttt{\bs{}isvar(?X,wish)}, the
...
\item \texttt{\bs{}isnonvar/2} --- the delayable version of
  \texttt{\bs{}isnonvar/1}.  If \texttt{?X} in
  \texttt{\bs{}isnonvar(?X,must)} \emph{at the end of the query computation} is a
  variable, an error is issued. Otherwise, the result is true.
  For \texttt{\bs{}isnonvar(?X,wish)}, the evaluation is delayed and then
  \texttt{?X} is tested. If it is bound, the result is true; otherwise,
  it is false. No errors are issued.
\item \texttt{\bs{}isground/2} --- the delayable version of
  \texttt{\bs{}isground/1}.  If \texttt{?X} in
  \texttt{\bs{}isground(?X,must)} \emph{at the end of the query computation}
  is non-ground, an error is issued. Otherwise, the result is true.
  For \texttt{\bs{}isground(?X,wish)}, the
  ...
  issued.
\item \texttt{\bs{}isnonground/2} --- the delayable version of
  \texttt{\bs{}isnonground/1}.  If \texttt{?X} in
  \texttt{\bs{}isnonground(?X,must)} \emph{at the end of the query computation}
  is ground, an error is issued. Otherwise, the result is true.
  For \texttt{\bs{}isnonground(?X,wish)}, the
  evaluation is delayed and then \texttt{?X} is tested. If it is bound to a
...
\index{\thismodule}
%
Since a \FLSYSTEM program can be loaded into any module, the program does not
have a priori knowledge of the module it will be executing in. However, the
program can determine its module at runtime using the quasi-constant
...
name, value, etc., in file {\tt foo.\flrext} then when {\tt foo.\flrext} is loaded
...
%%
\FLSYSTEM provides several commands for compiling and loading program files into
...
%%
\begin{quote}
  {\tt ?- \bs{}compile({\it file}>{}>{\it module}).}
\end{quote}
%%
...
\end{quote}
%%
compiles {\it file}.\flrext for the default module {\tt main}.
...
\index{module!{\tt \bs{}isloaded/1}}
\index{{\tt \bs{}isloaded/1}}
%%
...
predicate {\tt \bs{}isloaded/1}. For instance, the first query,
...
\paragraph{A word of caution.} It is dangerous to place the {\tt \bs{}load} command
in the body of a rule if {\tt \bs{}load} loads a file into the same module
...
dangerous occurrences of {\tt \bs{}load}, but it cannot intercept all such
cases reliably.
...
\subsection{Calling Prolog from \FLSYSTEM}\label{sec-prolog-modules}
...
\index{module!{\tt \bs{}prolog}}
\index{module!{\tt \bs{}prolog(modulename)}}
...
\index{module!{\tt \bs{}prologall}}
\index{module!{\tt \bs{}prologall(modulename)}}
...
predicates defined in \FLSYSTEM programs. To call predicates
defined in \FLSYSTEM programs, they must be imported by the Prolog program.

\subsubsection{Importing \FLSYSTEM Predicates into a Prolog Shell}
...
\item 
  ...
  \begin{quote}
    \tt
        ?- '\bs{}\bs{}flimport' {\it flora-predicate/arity} as {\it
      xsb-name}(\_,\_,...,\_)\\
...
\noindent
{\bf Note:} If \FLSYSTEM is installed outside of the XSB directory structure,
then you must let Prolog know the location of your installation of \FLSYSTEM.
This is done by executing the prolog instruction 
{\tt asserta(library\_directory(path-to-flora))}. For instance
%%
\begin{alltt}
 ?- asserta(library_directory('/home/me/\ENGINENAME')).  
\end{alltt}
%%
...
\subsubsection{Calling \FLSYSTEM from a Prolog Module}

To call \FLSYSTEM from within a Prolog program, say {\tt test.P}, the
...
The first form for {\tt '\bs{}\bs{}flimport'} above is used to both import the
predicate and also to load the program file defining it into a given \FLSYSTEM
user module. The second syntax is used when the \FLSYSTEM program is already
loaded into a module and we only need to import the corresponding
predicate.

\index{non-tabled predicates!importing into Prolog}
In {\tt '\bs{}\bs{}flimport'}, \emph{flora-predicate} is the name of the imported
predicate as it is known in the \FLSYSTEM module. For non-tabled predicates,
whose names start with {\tt \%} in \FLSYSTEM, \emph{flora-predicate} should
...
\end{verbatim}
%%
Note that the backslash is escaped here so that Prolog will interpret the
above as the backslashed 'flimport' rather than the character '\bs{}f'
followed by 'import'. This is also the reason why the backslash appears
twice in the various commands in the rest of this subsection below.
...
Prolog programs can also load and compile \FLSYSTEM programs using the following
queries (again,  {\tt \FLBOOTSTRAP} must be executed in advance): 
%%
\index{{\tt \bs{}load} operator}
\index{{\tt \bs{}compile} operator}
\begin{quote}
 \tt
 :- import '\bs{}\bs{}load'/1, '\bs{}\bs{}compile'/1 from flora2.\\
 ?- '\bs{}\bs{}load'({\it flora-file} >{}> {\it flora-module}).\\
 ?- '\bs{}\bs{}load'({\it flora-file}).\\
 ?- '\bs{}\bs{}compile'({\it flora-file} >{}> {\it flora-module}).\\
 ?- '\bs{}\bs{}compile'({\it flora-file}).
\end{quote}
%%
...
%%
\index{{\tt \bs{}isloaded/1} predicate}
\begin{quote}
 \tt
 :- import '\bs{}\bs{}isloaded'/1 from flora2.\\
 ?- '\bs{}\bs{}isloaded'({\it flora-module-name}).
\end{quote}
%%
Note that when used inside Prolog the {\tt \bs{}isloaded} predicate must be
quoted. Quoting is not necessary when it is used in \FLSYSTEM. 
...
program, then the best way is to start XSB and \FLSYSTEM using the {\tt
...
%%
\begin{enumerate}
  \item  If \FLSYSTEM is installed as a standalone application rather than an
    XSB package, then the \FLSYSTEM installation directory must be added to
    the XSB search path:
%%
\begin{alltt}
 ?- asserta(library_directory('/home/myHomeDir/\ENGINENAME')).  
\end{alltt}
%%
  \item The query
    %%
...
queries a \FLSYSTEM file, flrtest.\flrext:
%%
\begin{alltt}
...
\end{alltt}
%%
After the query to {\tt flrtest.\flrext} is successfully executed, the bindings
...
together and stored in {\tt foo.\flrext}.
Note, however, that when compiling {\tt foo.\flrext}, the compiler has no way
...
%%
\begin{alltt}
  #include "..\bs{}\bs{}foo\bs{}\bs{}bar.\flrext"  
\end{alltt}
%%
...
%% 
\begin{alltt}
     \$\{\bs{}neg a[b->c]@foo\} =.. [negation(neg), \${a[b->c]@main}]
     \$\{\bs{}neg p(a,b)@foo\} =.. [negation(neg), \${p(a,b)@main}]
     \$\{\bs{}naf a[b->c]@foo\} =.. [negation(naf), \${a[b->c]@main}]
     \$\{\bs{}naf p(a,b)@foo\} =.. [negation(naf), \${p(a,b)@main}]
\end{alltt}
%% 
...
%% 
\begin{verbatim}
     ${foo;bar} =.. [logic(or),${foo@main},${bar@main}]
     ${\if foo\then bar\else moo} =..
                    [control(ifthenelse),${foo@main},${bar@main},${moo@main}]
     ${\while foo \do bar} =..[control(whiledo),${foo@main},${bar@main}]
     ${insert{?V|cond(?V)}} =.. [update(insert),[?V],${cond(?V)@main}]
     ${deleteall{?V|cond(?V)}} =.. [update(deleteall),[?A],${cond(?A)@main}]
     ${?X=avg{?V|cond(?V)}} =.. [logic(and),${?T=count{?V|cond(?V)@main}},?X=?T]
     count{?V[?G]|cond(?V,?G)} =.. [aggregate(count),?V,[?G],${cond(?C,?G)@main},?R]
\end{verbatim}
%% 
...
\end{verbatim}
%%
However, that last feature is \emph{not} fully implemented in the sense
that the terms produced (if produced at all) on the left-hand side might
not be a callable ot otherwise valid \FLSYSTEM term.
...
terms that appear in a \FLSYSTEM program (unless they are marked with {\tt
...
\emph{well-founded semantics}, denoted \texttt{\bs{}naf}, 
\cite{gelder-alternating-89,gelder-ross-schlipf-91}; and \emph{explicit
  negation} \texttt{\bs{}neg}, which is analogous to what is called ``classical'' negation in
...
\index{\PLGNAF}
%%
\FLSYSTEM has \emph{three} operations for negation: \RULELOGNAF, \PLGNAF, and {\tt \bs{}neg}.
...
Normally, systems either require that the formula under \texttt{\bs{}naf} is
ground or they interpret something like \texttt{\bs{}naf p(?X)} as meaning ``not
...
differently, i.e., \texttt{\bs{}naf p(?X)}   should be interpreted as
\texttt{$\exists$?X \bs{}naf p(?X)}. Worse yet, if \texttt{?X} \emph{does} occur in the
rule head then it is confusing and error-prone to interpret
\texttt{h(?X) :- \bs{}naf p(?X)} as  \texttt{h(?X) :- \bs{}naf $\exists$?X p(?X)} using the Naf-Exists
semantics. And without the Naf-Exists semantics, \texttt{\bs{}naf p(?X)}  has no
meaning, if \texttt{?X} happens to be non-ground.

\FLSYSTEM takes a different approach. For body-only variables that appear
under \texttt{\bs{}naf} the semantics is Exists-Not. In addition, \texttt{\bs{}neg p}
implies \texttt{\bs{}naf p}. So,
%% 
\begin{verbatim}
   \neg p({1,2}).
   ?- \naf p(?X).
...
under \texttt{\bs{}naf} \emph{and} in the rule head, the semantics is also standard:
...
\end{verbatim}
%% 
the query succeeds because the call \texttt{\bs{}naf p(?X)} will be delayed past
...
for all \texttt{?X} (i.e., if \texttt{\bs{}naf $\exists$?X p(?X)} holds) then, in
particular, \texttt{$\exists$ \bs{}naf ?X p(?X)} is also true (assuming an
infinite number of constants) and the query
succeeds with the truth value \emph{true}.
If \texttt{$\forall$?X p(?X)} is true then \texttt{\bs{}naf $\exists$?X
  p(?X)}  is false.  
However, if none of the above cases apply then we are in a gray area
and there is not enough information to tell whether
\texttt{\bs{}naf $\exists$?X p(?X)} is true or false, so this subgoal
...
\texttt{\bs{}naf $\exists$?X p(?X)} is not true. On the other hand, the second
...
(fails). Therefore {\tt \PLGNAF\ Goal} intuitively means $\tt\forall?Vars\,\neg\,Goal$, where {\tt ?Vars} represents all the nonbound variables
in {\tt Goal}. However, here $\neg$ should be understood not as classical
negation but rather as a statement that \texttt{Goal}  cannot be proved to be true.
...
correctness of the result is not guaranteed. The expression {\tt
  false\{Goal\}} is equivalent to {\tt \bs{}naf Goal},  and can be used
anywhere.
...
%%
\emph{Explicit negation} is denoted using the connective {\tt \bs{}neg}, for
instance, {\tt \bs{}neg p}. It is a weaker form of classical negation and is
...
Prolog negation, in order to conclude {\tt \bs{}neg p} one must actually
\emph{prove} {\tt \bs{}neg p}---\emph{not} fail to prove {\tt p}. Also, unlike
classical negation, the law of excluded middle does not hold for {\tt \bs{}neg}.

The explicit negation connective, {\tt \bs{}neg}, can be applied to conjunctions
or disjunctions of literals.
\ifdef{\isfloraman}{
However, {\tt \bs{}neg p,q} is not allowed in the
rule head, since this is tantamount to a disjunction, {\tt \bs{}neg p \bs{}or \bs{}neg q}.  
}
In a rule body, the idioms {\tt \bs{}neg \bs{}naf p} and {\tt \bs{}neg \PLGNAF p} are
illegal, but {\tt \bs{}naf \bs{}neg p} and {\tt \PLGNAF \bs{}neg p} are legal and should be
informally understood as statements that {\tt p} is not known to be false. 

As with the default negation \RULELOGNAF, {\tt \bs{}neg} is allowed inside the
frames 
...
\end{verbatim}
%% 
are allowed, but {\tt a[\bs{}neg \%b]}, {\tt a[\bs{}neg b ->-> c]}, {\tt a[\bs{}neg b
  +> c]} are illegal.  
...
For instance, no consistency check is done to ensure that {\tt p} and {\tt
  \bs{}neg p} are not true at the same time. This is future work.
...
Transactional HiLog literals and methods cannot be negated using {\tt
  \bs{}neg}. For instance, the following literals are syntax errors:
%% 
\begin{verbatim}
    \neg %p
    \neg a[%p]
...
\section{Arbitrary First-order Formulas in Rule Bodies}\label{sec-lt}
...
formulas in the rule body. In due time we will see various if-the-else
clauses, loops, etc. But the free use of logical quantifiers \texttt{forall} and
\texttt{exists} is, perhaps the most unique. In fact, the rule bodies in 
\FLSYSTEM can have
arbitrary first-order formulas involving the quantifiers, \texttt{\bs{}and},
\texttt{\bs{}or}, \texttt{\bs{}naf}, and the logical implications \verb|~~>|,
\verb|<~~|, and \verb|<~~>|. Here are some examples:
%% 
\begin{verbatim}
   h1(?Z) :- \naf exists(?X)^(\naf ((p(?X) ~~> q(?X,Z)))).
   h2(?Y) :- forall(?X)^(p(?X,?Y) ~~> q(?X)). 
   h3(?V,?W) :- forall(?Y)^(\naf exists(?X)^(pp(?X,?W,?Y) , \naf qq(?X,?Y,?V))).
   ?- forall(?X)^(p(?X)<~~>p(?X)).
\end{verbatim}
%% 
Other connectives, such as \texttt{==>}, \texttt{if-then-else}, etc., 
can also be used in conjunction with the quantifiers. However, it makes no
sense to put non-logical operators in the scope of the quantifiers or
\texttt{\bs{}naf}. For instance,
%% 
\begin{verbatim}
   ?- \naf \load(abc).
\end{verbatim}
...
\end{verbatim}
%% 
will print out the contents of the predicate \texttt{data/1}. 

Quantifiers in \FLSYSTEM are implemented through the well-known Lloyd-Topor
transformation \cite{Lloy84} except that the role of negation here is
played by \texttt{\bs{}naf}, not \PLGNAF. However, unlike Prolog for which the
...
Quantifiers and \verb|~~>| have implicit negation in them, so, in order to
...
occurs in the scope of an even number of  \texttt{\bs{}naf}s. A variable occurs
...
To make the above more precise, we explain how to count the \texttt{\bs{}naf}s. This is
not immediate because of the implicit negations mentioned earlier.
So, for the purpose of counting the \texttt{\bs{}naf}s, one must break
each \texttt{forall(?Varlist)$\hat{}\phi$}  into
\texttt{\bs{}naf exists(?Varlist)$\hat{}$( \bs{}naf $\phi$)}. Each
$\phi$\verb|~~>|$\psi$ must be
broken into  \texttt{\bs{}naf $\phi$ or $\psi$}, and \verb|<~~| and
\texttt{if-then} are synonymous with \verb|~~>|, and   \verb|<~~>|
is a conjunction of \verb|~~>|, and \verb|<~~|. The connective
\texttt{\bs{}if $\phi$ \bs{}then $\psi$ \bs{}else $\gamma$} is treated for the purpose of
counting negations as   \texttt{$\phi$ \bs{}and \bs{}naf $\psi$ \bs{}or \bs{}naf $\gamma$}.  

Note that the connectives \texttt{==>}, \texttt{<==}, and \texttt{<==>} do
not involve \texttt{\bs{}naf}: they are translated using \texttt{\bs{}neg}. For
instance, \texttt{$\phi$==>$\psi$} is a shorthand for
\texttt{\bs{}neg $\phi$ or $\psi$}.  
...
One might be tempted to expect to inherit negative information such as this:
%% 
\begin{verbatim}
    c[|\neg attr->1|].
    obj:c.
...
\end{verbatim}
%% 
However, this does not make logical sense. The fact \texttt{c[|\bs{}neg att->1|]} says
...
nevertheless be achieved through a more powerful feature of defeasible
...
\end{verbatim}
%% 
which says that, by default, \verb|o[att->1]| is known to be false if
\texttt{o} is a member of class \texttt{c}. Other statements for the
members of class \texttt{c} may have higher priority and override the
above. For instance,
%% 
\begin{verbatim}
    abc:c.
    @{high} abc[att->1].
    \overrides(high,default(c)).
\end{verbatim}
%% 
This says that, for the member \texttt{abc} of class \texttt{c},  
\verb|att->1| is actually true because this information is specified with
higher priority (\texttt{high})  than the default rule
(\texttt{default(c)}) above, as indicates by the priority fact
\texttt{\bs{}overrides(high,default(c))}. 

Despite the above, negative information does play a role in inheritance. More
specifically, explicit negative information specified for subclasses and
class members is treated as explicit local statements that override
the inherited information. For instance, in
%% 
\begin{verbatim}
    c[|attr1->1, attr2->2, attr3->3|].
    cc::c.
    obj:cc.
    cc[|\neg attr1->abc|].
    obj[\neg attr2->cde].
    ?- obj[attr1->?X] or obj[attr2->?X].
    ?- cc[|attr1->?X|].
    ?- obj[attr3->?X].
    ?- cc[|attr2->?X|], cc[|attr3->?Y|].
\end{verbatim}
%% 
the explicit negative statement \texttt{cc[|\bs{}neg attr1->abc|]} blocks the
inheritance of \texttt{c[|attr1->1|]} down to both \texttt{cc} and \texttt{obj}.
Likewise, the explicit negative information \verb|obj[\neg attr2->cde]|
blocks the inheritance of \texttt{c[|attr2->2|]} down to \texttt{obj}. 
Therefore, the first two queries above return no answers.
For \texttt{attr3} (and for \texttt{attr2} in case of \texttt{cc}), however, no explicit blocking negative information
exists, so the last two queries succeed. 
...
semantic options by examining every loaded module via the {\tt
  \bs{}isloaded/1} built-in and then posing the desired {\tt
  semantics\{...\}} query.
...
XSB lets the user specify which predicates to table.  The \FLSYSTEM
compiler automatically tables frames and HiLog predicates. If the
...
When Prolog (and \FLSYSTEM) evaluate a program, all tabled predicates are
...
\index{refresh\{...\}}
%%
There are two ways to discard tabled information in \FLSYSTEM. One, and the
...
query {\tt ?- \bs{}abolishtables}.

{\tt \bs{}abolishtables} \emph{cannot} be used in the following cases:
%%
\begin{enumerate}
...
predicates. Execution of {\tt \bs{}abolishtables} in the body of such a
...
can depend via rules --- directly or indirectly --- on {\tt
  \bs{}abolishtables/0}.  However, it is safe to use this predicate in
...
or make updates.  In \FLSYSTEM we call such methods \emph{procedural}.
...
To sidestep this problem, \FLSYSTEM introduces a new syntax to identify
...
\subsubsection{Transactional Signatures}
...
\end{verbatim}
%% 
\FLSYSTEM does not support procedural methods specified as defaults at the
...
\subsection{Operational Semantics of \FLSYSTEM}\label{sec-flora-procedural}
...
\emph{procedural semantics} of \FLSYSTEM. This procedural semantics is
...
an already loaded module using the \texttt{\bs{}add}(\emph{ruleset})
...
the file \texttt{ruleset.\flrext} uses those functions, then it is necessary to
tell \FLSYSTEM \emph{in the file}   \texttt{ruleset.\flrext} that these are UDFs and not regular HiLog
...
will issue an error at loading time.\footnote{
  ...
  cannot know in advance that \texttt{ruleset.\flrext} is intended to
...
will be compiled correctly in \texttt{ruleset.\flrext} and
...
\index{refresh\{...\}}
%%
Nevertheless, \FLSYSTEM provides a partial solution to this problem in the form
...
{\tt \bs{}ignore\_depchk} directive, three warnings would be issued saying that
tabled literal {\tt t(?X,?Y)} depends on {\tt \%ins(?X)}, {\tt ?Y[\%close]},
and {\tt insert}. Notice that {\tt \bs{}ignore\_depchk \%ins(?\_)} tells the
compiler to ignore not only dependencies on {\tt \%ins/1}, but also all
dependencies that have {\tt \%ins/1} in the path.

The {\tt \bs{}ignore\_depchk} directive can also be used to ignore direct
...
\index{dependency checking!\bs{}warnings(compiler)}
%%
We should also mention here that executing the command
\texttt{\bs{}warnings(compiler)} in the \FLSYSTEM shell will turn the dependency
...
Rules that are inserted via the {\tt insertrule} and {\tt \bs{}add} commands
are called \emph{dynamic} and the rules loaded using the {\tt \bs{}load} or
...
using the {\tt clause} primitive. Instead, the unary predicate {\tt
  \bs{}isbasefact} should be used. For instance,
%% 
\begin{verbatim}
a:b.
?- \isbasefact(a:b).
...
\index{\verb|~~>|}
\index{\verb|<~~|}
The form {\tt if-then} also has the following alternative forms: \verb|~~>|
and \verb|<~~|.
...
{\tt (Action ; \bs{}true)} idiom in the loop body will definitely make the loop
...
hard-to-find error from creeping up into the user program.\footnote{
...
\paragraph{The \bs{}nontabled\_module directive.}
...
\item 
  ...
  using the \texttt{\bs{}add} command).
\item
...
F-sensors (defined using \FLSYSTEM syntax in \texttt{.\flrext} files).
%% 
\begin{verbatim}
...
\texttt{.\flrext} file and then loaded into a separate \FLSYSTEM module.
...
\index{rule Id}
Every rule in \FLSYSTEM is assigned a unique \emph{rule Id}.  A rule Id
...
static program rules and at run time for dynamic or reified rules.
...
possibly, other meta information) and the fact that such queries are not
...
\end{verbatim}
%% 
However, one must keep in mind that \texttt{\bs{}@F} occurring 
in the \FLSYSTEM shell is
different from \texttt{\bs{}@F} occurring in a file
(Section~\ref{sec-quasi-constants}) and that the file component of the rule
Id of a dynamically inserted rule has its own representation
...
Literals for the form \texttt{lit} and \texttt{\bs{}neg}  \texttt{lit} always oppose 
...
\index{\bs{}use\_argumentation\_theory directive}
%% 
\begin{alltt}
  :- \bs{}use_argumentation_theory.
  :- \bs{}use_argumentation_theory\{\emph{Module}\}.
\end{alltt}
%%
...
\texttt{myargth.\flrext}, then it can be declared in a \FLSYSTEM module
using
%% 
\begin{verbatim}
  :- \use_argumentation_theory{foo}.
...
construct such theories by analogy with GCL---see {\tt AT/flrgclp.\flrext}.  
...
form of {\tt \bs{}overrides} says the following. Let $\rho_1$ be a program
...
\end{alltt}
%%
The first form of the {\tt \bs{}opposes} predicate says that the atomic
...
(atomic HiLog formula, frame, ISA, or subclass), $L$ and {\tt
  \bs{}neg} $L$ always oppose each other, and there is no need to state this
...
They cannot be negated using {\tt \bs{}neg}, but the compiler does not check if
...
\end{verbatim}
%% 
at the top of a \FLSYSTEM module where defeasible reasoning is to be used.
This loads the new courteous argumentation theory into the module {\tt
  \bs{}gcl}. For the original courteous argumentation theory, place the
...
predicates {\tt \bs{}overrides} and {\tt \bs{}opposes}.  This means that the
...
other use of the non-logical or update features of \FLSYSTEM in defeasible
...
Extensive data type support is being planned for \FLSYSTEM in the future.
At present, \FLSYSTEM supports the built-in data types
{\tt \bs{}long}, \texttt{\bs{}integer},
{\tt \bs{}double},  \texttt{\bs{}decimal},
\texttt{\bs{}string}, {\tt \bs{}symbol}, {\tt \bs{}object}, {\tt
  \bs{}iri}, (international resource identifier), {\tt \bs{}time}, {\tt \bs{}date},
{\tt \bs{}dateTime}, and {\tt \bs{}duration}.
...
type. For instance, {\tt "2004-12-24"$\hat{~}\hat{~}$\bs{}date}, {\tt
  "2004-12-24T15:33:44"$\hat{~}\hat{~}$\bs{}dateTime}.
...
{\tt "12:33:55"$\hat{~}\hat{~}$\bs{}time} or even {\tt
  "12:33:55"$\hat{~}\hat{~}$\bs{}t}.
...
type to type. However, certain methods are more or less common:
 ...
 \begin{itemize}
    \item {\tt toSymbol}, which applies to a data type constant and
      ...
      {\tt "12:44:23"$\hat{~}\hat{~}$\bs{}time} then {\tt ?Y[toSymbol->'12:44:23']} will
      be true.
    \item {\tt toType}( parameters ), which applies to any class
      ...
      type. For instance, {\tt
        \bs{}time[toType(12,23,45) -> "12:23:45"$\hat{~}\hat{~}$\bs{}time]}.  The other will
      apply to the constant symbol representation of the data type. For instance,
      {\tt \bs{}time[toType('12:23:45')->"12:23:45"$\hat{~}\hat{~}$\bs{}time]}. 
    ...
    \item {\tt lessThan}( constant ), which tells when one constant is
...
The class \texttt{\bs{}symbol} has only the following method apart from the
...
\end{itemize}
%% 
Note that that the above is \emph{not} a class-level signature, but an
...
identifiers) is
{\tt "\emph{some iri}"$\hat{~}\hat{~}$\bs{}iri}, where \emph{literal} must have
a lexical form corresponding to IRIs on the World Wide Web. IRIs have the shorthand notation
{\tt \bs{}"\emph{some iri}"}, as mentioned before. The full IRI name of this
...
\end{verbatim}
%% 
This will bind {\tt ?X} to   {\tt \bs{}"http://www.w3c.org/a"}. 
Likewise,
%% 
\begin{verbatim}
  ?- ?Y = AAAWEB#"ab%20" 
\end{verbatim}
%% 
binds {\tt ?Y} to {\tt \bs{}"http://www.AAA.com/ab\%20"}. 
...
the \FLSYSTEM module {\tt \bs{}basetype}. They are described here by their
signatures.
%%
...
\paragraph{Examples:}
\begin{itemize}
\item      {\tt \bs{}"http://foo.bar.com/abc"} 
\item      {\tt "http://foo.bar.com/abc"$\hat{~}\hat{~}$\bs{}iri} 
\item      {\tt ?- \bs{}iri[toType('http://foo.bar.com/abc') ->}\\
  \hspace*{5cm}{\tt "http://foo.bar.com/abc"$\hat{~}\hat{~}$\bs{}iri]@\bs{}basetype}  
\item      {\tt ?- \bs{}"http://foo.bar.com/abc"$\hat{~}\hat{~}$\bs{}iri[host -> 'foo.bar.com']@\bs{}basetype}  
\end{itemize}
...
\begin{itemize}
\item {\tt
    \bs{}dateTime[toType(\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer,\bs{}decimal,
    \bs{}integer,\bs{}integer,\bs{}integer)
    => \bs{}dateTime]} \\
...
\begin{itemize}
      ...
      -> "1979-09-13T17:31:41-02:30"$\hat{~}\hat{~}$\bs{}dateTime]@\bs{}basetype}  
      \end{itemize}
...
The type name {\tt \bs{}date} has the following synonyms: 
{\tt \bs{}d},  {\tt 'http://www.w3.org/2001/XMLSchema\#date'}. 

The following methods are defined for this type and are available through
the system module {\tt \bs{}basetype}.
%%
\paragraph{Class methods:}
\begin{itemize}
    \item {\tt \bs{}date[toType(\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer) => \bs{}date]}
      \\
...
\begin{itemize}
  ...
  \item {\tt ?- "2001-11-23-02:30"$\hat{~}\hat{~}$\bs{}date[add("-P2Y2M10D"$\hat{~}\hat{~}$\bs{}duration) ->}\\
  \hspace*{7cm}{\tt
    "1979-09-13-02:30"$\hat{~}\hat{~}$\bs{}dt]@\bs{}basetype}.\\
...
\item {\tt ?- "12:44:55"$\hat{~}\hat{~}$\bs{}time[add("PT2M3S"$\hat{~}\hat{~}$\bs{}duration) -> "12:46:58"$\hat{~}\hat{~}$\bs{}time]@\bs{}basetype }\\
...
The following classes are available in module {\tt \bs{}basetype}.
...
\item
  {\tt \bs{}duration[toType(\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer,\bs{}integer,}
    \hspace*{5cm}{\tt \bs{}integer) => \bs{}duration]}
  \\
...
A synonym for the \texttt{\bs{}boolean}  type name is {\tt
  'http://www.w3.org/2001/XMLSchema\#boolean'}.

\index{class!\bs{}boolean}
All constants in this type belong to the built-in class {\tt \bs{}boolean}. 
The following methods are available in module {\tt \bs{}basetype}. 
%%
\paragraph{Class methods:}
%%
\begin{itemize}
...
same, i.e., {\tt "true"$\hat{~}\hat{~}$\bs{}boolean} and {\tt \bs{}true}
...
Doubles have a short form where the "..."$\hat{~}\hat{~}$\bs{}double wrapper is removed.

\FLSYSTEM treats floating point numbers as synonymous with {\tt \bs{}double} and
allows to use the {\tt \bs{}float} data type designator.  
...
\paragraph{Caveat:}  The long form and the short form should really be the
same, i.e., {\tt "2.50"$\hat{~}\hat{~}$\bs{}double} and {\tt 2.50}
must denote the same constant.  However, this has not been implemented yet.
In fact, it is not even possible to do arithmetic with the long
representation of doubles. To extract the number part from a
\texttt{\bs{}double}  data
type one should use the method {\tt rawValue}.  For instance,
{\tt ?- "1.2"$\hat{~}\hat{~}$\bs{}double[rawValue->?X]@\bs{}basetype}.
...
\paragraph{Caveat:} The long form and the short form must really be the
same, i.e., {\tt "123"$\hat{~}\hat{~}$\bs{}long} and {\tt 123} must denote the
...
not even possible to do arithmetic with the full representation of {\tt
  \bs{}long} 
integers. To extract the number part from a {\tt \bs{}long}  data type one should use
the method {\tt rawValue}.  For instance, {\tt ?-
  "12"$\hat{~}\hat{~}$\bs{}long[rawValue->?X]@\bs{}basetype}.



\subsection{ The Primitive Types $\bs{}$decimal and $\bs{}$integer}
...
types decimal and integer.  Instead, {\tt \bs{}decimal}  is a synonym for
{\tt \bs{}double} and {\tt \bs{}integer} for {\tt \bs{}long}.   
As usual, there are corresponding classes {\tt \bs{}integer} and {\tt
  \bs{}decimal}. 
...
\item {\tt \bs{}string[|concat(\bs{}string) => \bs{}string|]}  
...
\item {\tt \bs{}string[|=> startsWith(\bs{}string)|]}  
\item {\tt \bs{}string[|=> endsWith(\bs{}string)|]}  
\item {\tt \bs{}string[|substring(\bs{}integer,\bs{}integer) =>
    \bs{}string|]}  
  \\
...
The following methods are available from the standard module {\tt \bs{}basetype}: 
%%
\paragraph{Class methods:}
\begin{itemize}
...
In addition, it is possible to tell \FLSYSTEM to perform various run-time
...
in class {\tt Type} of module {\tt \bs{}typecheck}.  Its syntax is:
%% 
\begin{verbatim}
    ?- Type[check(?Atom,?Result)]@\typecheck.
\end{verbatim}
%% 
The {\tt ?Atom} variable must be bound to an atomic frame as
...
\item  If {\tt ?Atom} is of the form {\tt ?[|?Meth->?|]@?Mod} then only the
  consistency between {\tt ->} and {\tt =>} is checked only for the frames
...
or not. Similarly, \texttt{...[|...|]}-style  atoms indicates that
...
In contrast, \texttt{...[...]}-style atoms indicate that all type
...
{\tt =>}, but the atoms can have the \texttt{...[...]}-style or
...
The first (binary) form of \texttt{\bs{}tabledump}  above is equivalent to
...
%% 
\begin{verbatim}
   ?- \tabledump(userout,?,answers)@\plg(flrtabledump).
...
${?A(?B,?C)}[total_subgoals->3,
   ...
   ?- \tabledump(userout,p(2,?),answers)@\plg(flrtabledump).
...
\index{\bs{}set\_dump\_incomplete\_tables\_on\_exception/0}
%% 
\begin{verbatim}
...
\index{\bs{}dump\_incomplete\_tables\_after\_exception/1}
%% 
\begin{alltt}
...
prompt or include the query {\tt ?- \bs{}logforest} in the middle of a program. 
...
%%
\begin{alltt}
   ?- \bs{}logforest('foobar.\flrext').
\end{alltt}
%% 
will direct \FLSYSTEM to dump the entire forest log into the file {\tt foobar.\flrext}.
...
%%
\begin{alltt}
   ?- \bs{}logforest('foobar.\flrext', 'foobar.P').
\end{alltt}
%% 
will dump the trace in the above \FLSYSTEM format into the file {\tt foobar.\flrext}
...
\begin{alltt}
   ...
   ?- \bs{}logforest('foobar.\flrext', [table_abolished, error]).
\end{alltt}
%% 
...
\subsection{Controlling Call and Answer Depth, Timeouts, Unification
...
%% 
\begin{verbatim}
    ?- terminyzer(XSBTraceFile,SummaryFile)@\prolog(flrterminyzer).
\end{verbatim}
%% 
...
%%
\index{local\bs{}override!optimizer option}
%%
\begin{verbatim}
 local_override  
 class_expressions
\end{verbatim}
%%
Invoking {\tt \bs{}optimize(local\_override,somemodule)} can, in some cases,
...
Invoking {\tt \bs{}optimize(class\_expressions,somemodule)} always improves
...
The option {\tt \bs{}optimize(nonstrict\_subclasses,somemodule)} is, strictly
speaking, does not affect performance. It is more of a semantic option.
By default, \FLSYSTEM assumes that the subclass relationship \texttt{::}  is strict
...
%% 
\begin{itemize}
  \index{production\_mode compiler option}
\item  \texttt{production\_mode} - compile the file without various
  ...
  of rules.
  \index{development\_mode compiler option}
\item \texttt{development\_mode} - compile the file in the development
  ...
  requested from the \FLSYSTEM shell via the command
  \texttt{\bs{}production(on)} (see Section~\ref{sec-production-shell}).
...
%%
\begin{alltt}
  :- compiler\_options\{[production_mode,prolog(spec_repr)]\}.  
\end{alltt}
%%
...
\index{system module}
%%
\FLSYSTEM provides a number of useful libraries that other programs can use.
...
\FLSYSTEM library provides just a few common ones. See the XSB manual, volume
...
\FLSYSTEM keeps the facts that are part of the program or those that are
...
\begin{itemize}
  ...
  {\tt \FLUSERABORT} the program must contain the include statement
  %%
  \begin{verbatim}
...
This system module of \FLSYSTEM provides methods for testing cardinality
...
module defines the transactional method {\tt check} in class {\tt
  Cardinality} of module {\tt \bs{}typecheck}.  This method is described in
Section~\ref{sec-cardinality}.

\subsection{Data Types}\label{sec-data-types-sysmod}

This system module of \FLSYSTEM provides methods for accessing the components
...
%% 
\paragraph{$\bullet$~~~ \tt ?- read(?Code,?Mod,?Stat)@\bs{}parse.} ~~\\
...
\end{verbatim}
  %% 
%% 
  \paragraph{$\bullet$~~~ \tt ?- readAll(?Code,?Mod,?Stat)@\bs{}parse.} ~~\\
  ...
  \paragraph{$\bullet$~~~\tt ?- ?Source[readAll(?Module,?CodeList)]@\bs{}parse.}~~\\
...
Programming in \FLSYSTEM is similar to programming in Prolog, but is more
...
%% 
\begin{alltt}
   :- compiler\_options\{[production_mode]\}.
\end{alltt}
%% 
...
the prompt:
\index{\bs{}production(on)}
\index{\bs{}production(off)}
%% 
\begin{verbatim}
   ?- \production(on).
\end{verbatim}
...
any file, so \texttt{\bs{}production(on)} must be re-executed each time
...
\texttt{\bs{}production(on)/\bs{}production(off)} commands. 
...
Prolog program more readable. The {\tt \bs{}dump/1}  predicate can be used to strip the
...
If you find a Prolog bug triggered by a \FLSYSTEM program, here is a set of
guidelines that can simplify the job of the XSB developers and increase the
chances that the bug will be fixed promptly:
%%
\begin{enumerate}
  \item  Reduce the size of your \FLSYSTEM program as much as possible, while
  ...
  \item  Use {\tt \bs{}dump/1} to strip \FLSYSTEM macros from the output of the
    \FLSYSTEM compiler.
...
BodyNegative := (('\bs{}naf' | '\bs{}neg | '\ensuremath{\backslash}+') Body)  |  'false\{' Body '\}'
BodyConstraint := '\{' CLPR-style constraint '\}'

ControlFlowStatement  := IfThenElse | UnlessDo
                          | WhileDo | WhileLoop
                          | DoUntil | LoopUntil
IfThenElse := '\bs{}if' Body '\bs{}then' Body  ('\bs{}else' Body)?  | Body '<-' Body
...
SpecBody := ('\bs{}naf')? MethodSpecification
SpecBody := '\bs{}neg'    ExplicitlyNegatableMethodSpecification
...
Editing and debugging \FLSYSTEM programs can be greatly simplified with the
help of \emph{flora-mode}, a special Emacs editing mode designed
specifically for \FLSYSTEM programs. Flora-mode provides support for syntactic
highlighting, automatic indentation, and the ability to run \FLSYSTEM programs
...
%%
\begin{quote}
  {\tt .../\ENGINENAME/emacs/flora.el} 
\end{quote}
%%
found in your \FLSYSTEM distribution on the load path of Emacs or XEmacs
...
Finally, you must tell X/Emacs how to recognize \FLSYSTEM program files, so
Emacs will be able to invoke the Flora major mode automatically when you
are editing such files:
%%
\begin{alltt}
(setq auto-mode-alist (cons '("\\.\flrext$" . flora-mode) auto-mode-alist))
...
By default, {\tt \bs{}load(myprog)} loads the program into the default user
...
the command \texttt{?-~\bs{}demo(default).}, pose the following query
