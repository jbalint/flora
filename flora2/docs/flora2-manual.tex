...
%%
\begin{alltt}
   cd \textnormal{\emph{path-to-the-directory-where-you-unpacked-flora2}}
   makeflora clean
   makeflora \textnormal{\emph{path-to-prolog-executable}}
\end{alltt}
%%
...
\noindent
\textbf{Note:} You might run into a problem if either
\emph{path-to-the-directory-where-you-unpacked-flora2} or
\emph{path-to-prolog-executable} has spaces anywhere in their names.
You can try to work around this problem by substituting each space
character with its encoding \texttt{\%20}. For instance, 
%%
\begin{verbatim}
   makeflora  C:\Program%20Files\My%20Dir\bin\xsb
\end{verbatim}
%%
If this does not work, move both XSB and \FLORA or a directory that has no
spaces (neither in its name nor in its parents' names).
\hfill $\Box$
...
%%
\begin{quote}
{\tt "C:$\backslash$Documents and Settings$\backslash$\textnormal{\emph{your-user-name}}"} 
\end{quote}
%%
...
%%
\begin{alltt}
   makeflora -c \textnormal{\emph{path-to-prolog-executable}}
\end{alltt}
%%
(The same remark regarding spaces in
\textnormal{\emph{path-to-prolog-executable}} applies.)
...
If you are using a 64-bit version of XSB on Windows and wanted to recompile
\FLORA, you must use the \texttt{makeflora -c64} command instead of
\texttt{makeflora -c}. In that case, you need to find the file
...
\begin{alltt}
   cd \textnormal{\emph{path-to-the-directory-where-you-unpacked-flora2}}
   make clean
   ./makeflora all \textnormal{\emph{path-to-prolog-executable}}
\end{alltt}
%%
...
Both \PLGNAF and \SILKNAF can be used as operators inside and outside of
...
As with the default negation \SILKNAF, {\tt neg} is allowed inside the
frames 
...
\paragraph{The current rule id token,  \texttt{\_@!}.} 
\index{\_@!, current rule Id token}
\index{current rule Id token, \_@!}
