...
way to see the internals, while \texttt{dbgout/1} and \texttt{dbgoutln/1}
(in the \texttt{\bs{}io} module) 
can be seen as intermediate devices. These are similar to
\texttt{write(...)@\bs{}io} and \texttt{writeln(...)@\bs{}io} except that
the Skolems are written out using the internal representation. Other
similar features might be added later. For instance,
%% 
\begin{verbatim}
  ?- insert{p(\#)}, p(?_X), writeln(?_X)@\io, dbgoutln(?_X)@\io.
  \#
