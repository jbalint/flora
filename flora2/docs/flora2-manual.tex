...
The statements for which the \texttt{?Variable =.. ?List} mode has not been
implemented includes clause\{...\}, \texttt{newmodule\{...\}},
\texttt{catch\{...\}}, the delay quantifiers, and some others. The
statements for which \texttt{=..} is not fully reversible are the update
operators. For instance, while
%% 
\begin{verbatim}
  ?- ${insert{p,(q:-r)}} =.. [update(insert),main,[${p}, ${q :- r}]].
\end{verbatim}
