...
If xsb is aliased to the XSB executable of your XSB installation or if
\emph{path-for-/XSB/bin/xsb}  is on the system search path for
executables (the \texttt{PATH} environment variable), then
the part \emph{path-for-/XSB/bin/xsb}  can be omitted.

\paragraph{Windows.}
For Windows, type:
%% 
\begin{alltt}
   floraconfig  \textnormal{\emph{path-for-{\ensuremath\backslash}XSB{\ensuremath\backslash}bin{\ensuremath\backslash}xsb.bat}}
\end{alltt}
%% 
or
%% 
\begin{alltt}
   floraconfig  \textnormal{\emph{path-for-{\ensuremath\backslash}XSB{\ensuremath\backslash}bin{\ensuremath\backslash}xsb64.bat}}
\end{alltt}
%% 
depending on whether you installed XSB as a 32-bit or a 64-bit application.

\paragraph{Starting \FLORA.}
If everything went well, the command \texttt{./runflora} (Linux/Mac/Cygwin)
or \texttt{runflora.bat} (Windows) can be used to start the FLORA-2 command
line shell.

\textbf{Note:} you may notice that during the first few uses of the systems
...
changes file timestamps during the unzipping process.


\subsection{Installing \FLORA from SVN}

If you downloaded \FLORA from its SVN repository at
\url{https://sourceforge.net/p/flora/src/959/tree/}, follow this
procedure. It requires that the typical development software is
installed. On Unix-based systems, this includes the GCC compiler, Make,
and Autoconf. On Windows, this means Microsoft Visual Studio Express (which
is a free download) or a full (paid) Studio version.

\paragraph{Linux, Mac, and Cygwin.}
Downloaded the latest development release
from SVN into a separate directory \emph{outside}
the XSB installation tree. The
\FLORA sources will be placed in the {\tt flora2} subdirectory of the
...
\begin{alltt}
   cd flora2
   make clean
   ./makeflora all \textnormal{\emph{path-for-/XSB/bin/xsb}}
\end{alltt}
%%
If the XSB executable is on your program search {\tt PATH}, then in the
third command above you can omit all arguments and simply type
\texttt{./makeflora}. 
...
We will use the above in our examples.
...
%%
\begin{alltt}
   cd \textnormal{\emph{flora2}}
   makeflora clean
   makeflora \textnormal{\emph{path-for-{\ensuremath\backslash}XSB{\ensuremath\backslash}bin{\ensuremath\backslash}xsb.bat}}
\end{alltt}
%%
Here \emph{flora2}  is the root directory 
for \FLORA in your SVN checkout; it should have the form
\emph{something}$\backslash${\tt flora2}. The argument
\textnormal{\emph{path-for-{\ensuremath\backslash}XSB{\ensuremath\backslash}bin{\ensuremath\backslash}xsb.bat}}
must be the full path name for the XSB invocation script.
Note, that unlike Unix, there should be no ``all'' after
``makeflora'' in Windows.

\noindent
\textbf{64-bit version:}  
If your XSB version is 64-bit, use \texttt{xsb64.bat} above instead of
\texttt{xsb.bat}.  
...
\texttt{vcvarsx86\_amd64.bat} or \texttt{SetEnv.cmd} and execute it.  
However, as noted, this recompilation is not necessary unless you modify
...
knowledge base requires a lot of main memory.
...
In contrast, {\tt bagof} and {\tt setof} succeed even if {\tt
...
\end{verbatim}
  %% 
  \index{\_@!, current rule Id token}
  \index{current rule Id token, \_@!}
