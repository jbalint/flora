...
%% This block should be adjusted for Ergo
...
\newcommand{\FLSETTIMEOUT}{flora\_set\_timeout}
\newcommand{\FLCLEARTIMEOUT}{flora\_clear\_timeout}
\newcommand{\FLSETSUBGOALDEPTH}{flora\_set\_subgoal\_depth}
\newcommand{\FLSETANSWERDEPTH}{flora\_set\_answer\_depth}
\newcommand{\FLSETUNIFICATIONMODE}{flora\_set\_unification\_mode}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{alltt}
   cd flora2
   makeflora clean
   makeflora \textnormal{\emph{path-to-}}{\ensuremath\backslash}XSB{\ensuremath\backslash}bin{\ensuremath\backslash}xsb.bat
\end{alltt}
%%
...
\textnormal{\emph{path-for-}}\texttt{{\ensuremath\backslash}XSB{\ensuremath\backslash}bin{\ensuremath\backslash}xsb.bat}
...
%%
\begin{alltt}
   makeflora -c \textnormal{\emph{path-for-}}{\ensuremath\backslash}XSB{\ensuremath\backslash}bin{\ensuremath\backslash}xsb.bat
\end{alltt}
%%
...
\texttt{.flr} suffix is not required)  and that file contains
%% 
\begin{verbatim}
   writeln(Welcome!)@_plg.
   insert{foo(bar)}.
\end{verbatim}
%% 
...
%%
\begin{alltt}
  \prompt _load(program).
  \prompt _load(url(program)).
\end{alltt}
%%
...
If {\tt program.flr} exists, it is assumed to be a \FLSYSTEM program. The
...
%% 
\begin{alltt}
  \prompt _load(url('http://example.com/test1')).
...
of looking for the program file {\tt \_.flr}, \FLSYSTEM starts reading user
...
\FLSYSTEM's \texttt{\_load}  and \texttt{\_add} commands try to be smart in
...
system uses the file with the given name without any modification. The {\tt
  .flr} suffix denotes a \FLSYSTEM program, the {\tt .P} suffix indicates that
...
is dealing with a \FLSYSTEM program and adds the suffix {\tt .flr}. If the
...
\item {\tt abolish\_all\_tables}:
    ...
    gets in the way. We describe tabling (as it pertains to \FLSYSTEM) in
...
command {\tt \_demo(FILE)} first looks for {\tt FILE} in the \FLSYSTEM demo
...
\section{\fl and \FLSYSTEM by Example}
...
reader can read the \FLSYSTEM tutorial, which is
...
\url{http://www.ontoprise.de/documents/tutorial_flogic.pdf}.  \FLSYSTEM shares
much of the syntax with those other systems with the following notable
differences: \FLSYSTEM uses ``,'' as the separator between methods in object
formulas, while these other systems use ``;''. In addition, \FLSYSTEM does not
use the @-sign between method names and arguments.


\section{Differences Between the \FLSYSTEM Syntax and the F-logic Syntax}

\FLSYSTEM was developed several years after the publication of the initial
...
\item In \cite{KLW95}, types are always inheritable, but values are not.
  ...
  subclasses of {\tt a}, but the property {\tt a[b*->c]} is.  In \FLSYSTEM,
  the notation for types is brought in line with the notation for values.
  In particular, \FLSYSTEM uses {\tt *=>} for inheritable types
...
\item The type inference rules for input restriction and output
  relaxation introduced in \cite{KLW95} are not implemented in \FLSYSTEM.
\item The syntax {\tt a[b=>\{c,d\}]} of \fl, which states that the type
  ...
  $\tt d$, is not allowed. Use {\tt a[b=>(c,d)]} instead. \FLSYSTEM also allows
...
e.g., \texttt{'JOHN SMITH'},\texttt{'default.flr'}.  Internally, \FLSYSTEM
...
All \FLSYSTEM symbols belong to the class {\tt \_symbol}.
...
expression, \FLSYSTEM provides another operator, {\tt is}.  For example, {\tt
...
%% 
\begin{itemize}
\item  \texttt{\_\#}, \texttt{\_\#1}, \texttt{\_\#2}, etc. -- Skolem constants.  
\item  \texttt{\_!@} -- the Id of the rule where this quasi-constant occurs.
\item  \texttt{\_@} -- the module into which the file containing this
  quasi-constant is loaded. In the \FLSYSTEM shell, this quasi-constant is set
  to be the atom \texttt{main}. 
\item \texttt{\_@F} -- the file in which this quasi-constant occurs. In the
  \FLSYSTEM shell, this quasi-constant is set to be the atom
  \texttt{'(interactive)'}. 
\item \texttt{\_@L} -- the line in the source code on which this
...
%% 
\begin{itemize}
\item \texttt{\_?C}  -- the module from which the given rule was called.
  Used in the bodies of rules and queries.
\item \texttt{\_?F} -- the file name from which a sensor was called.
  Used only in the bodies of the rules that are part of a sensor definition.
\item \texttt{\_?L} -- the line number from which a sensor was called.
  Used only in the bodies of the rules that are part of a sensor definition.
\end{itemize}
%% 
...
\FLSYSTEM has a number of reserved keywords, which cannot be used directly \emph{as
...
\begin{alltt}
   ?- do(r).
   ++Error[\errorsystem]> <Parser> near line(1)/char(4) `r'
...
(of {\tt c} and {\tt d}) is not permitted.
...
\FLSYSTEM: a formula is compiled into an object that represents that formula if
...
quazi-constants \verb|_#| and \verb|_#1|, \verb|_#2|, etc.,
...
\emph{numbered Skolem symbols}, which are of the form \verb|_#132|, {\it
...
\end{verbatim}
%% 
the four occurrences of \texttt{\_\#1} represent the same Skolem constant.
Note that the first and the last statement above are rules, while the two
middle statements are facts. The reason why \texttt{\_\#1} is because all
...
\end{verbatim}
%% 
Here \texttt{\_\#1} inside \texttt{p(...)}  is a constant, while in 
...
predicate symbol.
For instance,
{\tt \_\#(a,b)}. Since \FLSYSTEM terms are actually HiLog terms, we can also
...
To this end, \FLSYSTEM provides a special built-in class, {\tt \_skolem}. 
...
\texttt{\_isskolem/1} meta-predicate, which can tell whether a particular
...
\begin{itemize}
  \index{\_isnumber/1}
\item \texttt{\_isnumber/1} --- tests whether the argument is (or is bound
  to) a number.
  \index{\_isinteger/1}
\item \texttt{\_isinteger/1} --- tests whether the argument is (or is bound
  to) an integer number. 
  \index{\_isfloat/1}
\item \texttt{\_isfloat/1} --- tests whether the argument is (or is bound
  to) a floating point number.
  \index{\_isdecimal/1}
\item \texttt{\_isdecimal/1} --- tests if the argument is (or is bound
  to) a decimal number. At present, this is the same as \texttt{\_isnumber/1}.
  \index{\_isatom/1}
\item \texttt{\_isatom/1} --- tests whether the argument is (or is bound
  to) a Prolog atom.
  \index{\_iscompound/1}
\item \texttt{\_iscompound/1} --- tests whether the argument is (or is bound
  to) a compound term, i.e., a term that has a non-zero-ary function
  symbol.
  \index{\_isatomic/1}
\item \texttt{\_isatomic/1} --- tests if the argument is (or is bound
  to) a Prolog atom or a number.
  \index{\_islist/1}
\item \texttt{\_islist/1} --- tests if the argument is (or is bound
  to) a list term.
  \index{\_ischarlist/1}
\item \texttt{\_ischarlist/1} --- tests whether the argument is (or is bound
  to) a list or ASCII characters.
  \index{\_isiri/1}
\item \texttt{\_isiri/1} --- tests whether the argument is (or is bound
  to) an IRI data type.
  \index{\_isstring/1}
\item \texttt{\_isstring/1} --- tests whether the argument is (or is bound
  to) a string data type.
  \index{\_issymbol/1}
\item \texttt{\_issymbol/1} --- tests whether the argument is (or is bound
  ...
  \index{\_isvar/1}
\item \texttt{\_isvar/1} --- tests if the argument is an unbound variable.
  \index{\_isnonvar/1}
\item \texttt{\_isnonvar/1} --- tests if the argument is not an unbound variable.
  \index{\_isground/1}
\item \texttt{\_isground/1} --- tests if the argument is a ground term (or
  is bound to one).
  \index{\_isnonground/1}
\item \texttt{\_isnonground/1} --- tests if the argument is not a ground term.
\end{itemize}
%% 
...
\prompt _isinteger(?X), ?X=1.

No

\prompt _isinteger(?X,wish), ?X=1.

?X = 1

Yes

\prompt _isinteger(?X,must), ?X=1.

?X = 1

Yes

\prompt _isinteger(?X,must).

++Abort[\errorsystem]> in file (runtime) on line 1: instantiation error in builtin:
                _isinteger(_h3464); unbound argument

\prompt _isinteger(?X,wish).
...
%% 
\begin{verbatim}
  ... :- ..., _isinteger(?X,must), foo(?X), ...
\end{verbatim}
%% 
will not prevent calling \texttt{foo/1} with \texttt{?X} an unbound
variable,
since \texttt{\_isinteger/2} will be delayed past the moment
\texttt{foo(?X)} is evaluated. Using \texttt{\_isinteger(?X)} instead will
do the job.

The delayable versions of the aforesaid builtins are listed below. The
delayable versions of
\texttt{\_isvar/2}, \texttt{\_isnonvar/2}, \texttt{\_isground/2}, and
\texttt{\_isnonground/2} have slightly different semantics.
%% 
\begin{itemize}
  \index{\_isnumber/2}
\item \texttt{\_isnumber/2} --- the delayable version of \texttt{\_isnumber/1}.
  \index{\_isinteger/2}
\item \texttt{\_isinteger/2} --- the delayable version of \texttt{\_isinteger/1}.
  \index{\_isfloat/2}
\item \texttt{\_isfloat/2} --- the delayable version of \texttt{\_isfloat/1}.
  \index{\_isdecimal/2}
\item \texttt{\_isdecimal/2} --- the delayable version of \texttt{\_isdecimal/1}.
  \index{\_isatom/2}
\item \texttt{\_isatom/2} --- the delayable version of \texttt{\_isatom/1}.
  \index{\_iscompound/2}
\item \texttt{\_iscompound/2} --- the delayable version of \texttt{\_iscompound/1}.
  \index{\_isatomic/2}
\item \texttt{\_isatomic/2} --- the delayable version of \texttt{\_isatomic/1}.
  \index{\_islist/2}
\item \texttt{\_islist/2} --- the delayable version of \texttt{\_islist/1}.
  \index{\_ischarlist/2}
\item \texttt{\_ischarlist/2} --- the delayable version of \texttt{\_ischarlist/1}.
  \index{\_isiri/2}
\item \texttt{\_isiri/2} --- the delayable version of \texttt{\_isiri/1}.
  \index{\_isstring/2}
\item \texttt{\_isstring/2} --- the delayable version of \texttt{\_isstring/1}.
  \index{\_issymbol/2}
\item \texttt{\_issymbol/2} --- the delayable version of \texttt{\_issymbol/1}.
  \index{\_isvar/2}
\item \texttt{\_isvar/2} --- the delayable version of \texttt{\_isvar/1}.
  If the argument \texttt{?X} in \texttt{\_isvar(?X,must)} \emph{at the end of
  the query computation} is \emph{not} an unbound
  variable, an error is issued. Otherwise, the result is true.
  For \texttt{\_isvar(?X,wish)}, the
...
\item \texttt{\_isnonvar/2} --- the delayable version of
  \texttt{\_isnonvar/1}.  If \texttt{?X} in
  \texttt{\_isnonvar(?X,must)} \emph{at the end of the query computation} is a
  variable, an error is issued. Otherwise, the result is true.
  For \texttt{\_isnonvar(?X,wish)}, the evaluation is delayed and then
  \texttt{?X} is tested. If it is bound, the result is true; otherwise,
  it is false. No errors are issued.
\item \texttt{\_isground/2} --- the delayable version of
  \texttt{\_isground/1}.  If \texttt{?X} in
  \texttt{\_isground(?X,must)} \emph{at the end of the query computation}
  is non-ground, an error is issued. Otherwise, the result is true.
  For \texttt{\_isground(?X,wish)}, the
  ...
  issued.
\item \texttt{\_isnonground/2} --- the delayable version of
  \texttt{\_isnonground/1}.  If \texttt{?X} in
  \texttt{\_isnonground(?X,must)} \emph{at the end of the query computation}
  is ground, an error is issued. Otherwise, the result is true.
  For \texttt{\_isnonground(?X,wish)}, the
  evaluation is delayed and then \texttt{?X} is tested. If it is bound to a
...
Alternatively, the \texttt{\_?C} quasi-variable can be used. 
For instance, 
%%
\begin{verbatim}
 p(?X) :- caller{?X}, (write('I was called by module: '), writeln(?X))@_prolog.
 p(?X) :- ?X=_?C, (write('I was called by module: '), writeln(?X))@_prolog.
\end{verbatim}
%%
...
\end{quote}
%%
This loads the byte code of the \FLSYSTEM program {\tt myprog.flr} into the user module named {\tt
...
{\it file}.{\tt flr} exists. If so, the file assumed to be a \FLSYSTEM program
...
\verb|[_>>module]|, \verb|[+_]|, and \verb|[+_>>module]|. This causes \FLSYSTEM to start reading program clauses
...
queries a \FLSYSTEM file, flrtest.flr:
%%
\begin{alltt}
...
process the Prolog terms into which \FLSYSTEM literals are encoded internally
...
\emph{well-founded semantics}, denoted \texttt{naf}, 
\cite{gelder-alternating-89,gelder-ross-schlipf-91}; and \emph{explicit
  negation} \texttt{neg}, which is analogous to what is called ``classical'' negation in
...
\subsection{Default Negation \texttt{naf}  vs.~Prolog Negation \PLGNAF}\label{sec-negation}
\index{\RULELOGNAF}
\index{\PLGNAF}
%%
\FLSYSTEM has \emph{three} operations for negation: \RULELOGNAF, \PLGNAF, and {\tt neg}.
...
Normally, systems either require that the formula under \texttt{naf} is
ground or they interpret something like \texttt{naf p(?X)} as meaning ``not
...
differently, i.e., \texttt{naf p(?X)}   should be interpreted as
\texttt{$\exists$?X naf p(?X)}. Worse yet, if \texttt{?X} \emph{does} occur in the
rule head then it is confusing and error-prone to interpret
\texttt{h(?X) :- naf p(?X)} as  \texttt{h(?X) :- naf $\exists$?X p(?X)} using the Naf-Exists
semantics. And without the Naf-Exists semantics, \texttt{naf p(?X)}  has no
meaning, if \texttt{?X} happens to be non-ground.

\FLSYSTEM takes a different approach. For body-only variables that appear
under \texttt{naf} the semantics is Exists-Not. In addition, \texttt{neg p}
implies \texttt{naf p}. So,
%% 
\begin{verbatim}
   neg p({1,2}).
   ?- naf p(?X).
...
under \texttt{naf} \emph{and} in the rule head, the semantics is also standard:
...
\end{verbatim}
%% 
the query succeeds because the call \texttt{naf p(?X)} will be delayed past
...
for all \texttt{?X} (i.e., if \texttt{naf $\exists$?X p(?X)} holds) then, in
particular, \texttt{$\exists$ naf ?X p(?X)} is also true (assuming an
infinite number of constants) and the query
succeeds with the truth value \emph{true}.
If \texttt{$\forall$?X p(?X)} is true then \texttt{naf $\exists$?X
  p(?X)}  is false.  
However, if none of the above cases apply then we are in a gray area
and there is not enough information to tell whether
\texttt{naf $\exists$?X p(?X)} is true or false, so this subgoal
...
\texttt{naf $\exists$?X p(?X)} is not true. On the other hand, the second
...
Prolog negation, in order to conclude {\tt neg p} one must actually
...
\FLSYSTEM can have
arbitrary first-order formulas involving the quantifiers, \texttt{and},
\texttt{or}, \texttt{naf}, and the logical implications \verb|~~>|,
\verb|<~~|, and \verb|<~~>|. Here are some examples:
%% 
\begin{verbatim}
   h1(?Z) :- naf exists(?X)^(naf ((p(?X) ~~> q(?X,Z)))).
   h2(?Y) :- forall(?X)^(p(?X,?Y) ~~> q(?X)). 
   h3(?V,?W) :- forall(?Y)^(naf exists(?X)^(pp(?X,?W,?Y) , naf qq(?X,?Y,?V))).
   ?- forall(?X)^(p(?X)<~~>p(?X)).
\end{verbatim}
%% 
...
played by \texttt{naf}, not \PLGNAF. However, unlike Prolog for which the
...
occurs in the scope of an even number of  \texttt{naf}s. A variable occurs
...
To make the above more precise, we explain how to count the \texttt{naf}s. This is
not immediate because of the implicit negations mentioned earlier.
So, for the purpose of counting the \texttt{naf}s, one must break
each \texttt{forall(?Varlist)$\hat{}\phi$}  into
\texttt{naf exists(?Varlist)$\hat{}$( naf $\phi$)}. Each
$\phi$\verb|~~>|$\psi$ must be
broken into  \texttt{naf $\phi$ or $\psi$}, and \verb|<~~| and
\texttt{if-then} are synonymous with \verb|~~>|, and   \verb|<~~>|
is a conjunction of \verb|~~>|, and \verb|<~~|. The connective
\texttt{if $\phi$ then $\psi$ else $\gamma$} is treated for the purpose of
counting negations as   \texttt{$\phi$ and naf $\psi$ or naf $\gamma$}.  

Note that the connectives \texttt{==>}, \texttt{<==}, and \texttt{<==>} do
not involve \texttt{naf}: they are translated using \texttt{neg}. For
instance, \texttt{$\phi$==>$\psi$} is a shorthand for
\texttt{neg $\phi$ or $\psi$}.  
...
%%
\fl (and \FLSYSTEM) distinguishes between attributes and methods that can
...
therefore the inherited value would be of no use. In \FLSYSTEM, we would
...
In \FLSYSTEM, such conflicts cause previously established inheritance
...
attributes from non-inheritable ones, \FLSYSTEM uses the symbol
...
\end{verbatim}
%% 
However, this does not make logical sense. The fact \verb|c[neg att*->1]| says
...
that it is known that \verb|att->1| is false for \texttt{obj} or that 
...
(\texttt{c\_default}) above, as indicates by the priority fact
\texttt{\_overrides(high,c\_default)}. 
...
class members is considered to be explicit local statements that override
...
\end{verbatim}
%% 
the explicit negative statement \verb|neg attr1*->abc| blocks the
inheritance of \verb|attr1*->1| down to both \texttt{cc} and \texttt{obj}.
Likewise, the explicit negative information \verb|neg attr2->cde|
blocks the inheritance of \verb|attr2*->2| down to \texttt{obj}. 
...
%%
\FLSYSTEM enables the user to choose the appropriate semantics for any user
...
\end{verbatim}
%% 
\FLSYSTEM does not support inheritable procedural methods at present, but the
...
tell \FLSYSTEM \emph{in the file}   \texttt{ruleset.flr} that these are UDFs and not regular HiLog
...
\texttt{\_warnings(compiler)} in the \FLSYSTEM shell will turn the dependency
...
sets. For this, \FLSYSTEM provides another pair of primitives: {\tt +>>} and
...
\paragraph{The nontabled\_module directive.}
...
\item
  ...
  \texttt{.flr} files using the \FLSYSTEM syntax (F-sensors). 
...
F-sensors (defined using \FLSYSTEM syntax in \texttt{.flr} files).
%% 
\begin{verbatim}
...
\texttt{.flr} file and then loaded into a separate \FLSYSTEM module.
...
\texttt{\_?F} and \texttt{\_?L}. When these quasi-variables occur in the body of a
sensor definition, \texttt{\_?F} is replaced with the file name from which
the sensor was called; the quasi-variable \texttt{\_?L} is replaced with the line
...
using the usual \texttt{table} directive. \FLSYSTEM provides a similar
...
%% 
\begin{alltt}
++Abort[\errorsystem]> in file foo.flr, line 3: unsatisfied must-condition.
	Goal: q(?A)
	Condition: nonvar(?A)
\end{alltt}
%% 
...
\item \emph{file\_name} is the file name where the rule occurs. The name
  ...
  extensions (e.g., \texttt{foo.flr}). The \texttt{\#include} commands are
...
\end{verbatim}
  %% 
  \index{\_@!, current rule Id quasi-constant}
  \index{current rule Id quasi-constant, \_@!}
  Without these shortcuts, the user would have to use the \emph{current rule Id
  quasi-constant}, \texttt{\_@!}, which gets substituted for the Id of the rule
in which it occurs:
%% 
\begin{verbatim}
...
quasi-constant
\texttt{\_@!}. This quasi-constant gets replaced by the \emph{local part}
...
To get the file name part, use \texttt{\_@F} and the module
part use \texttt{\_@} --- the quasi-constants introduced in
...
shortcut for the three-argument version, where argument 2 is \texttt{\_@F}
and argument 3 is \texttt{\_@}. For instance,
\texttt{tenable\{foo\}} instead of
\texttt{tenable\{foo,\_@F,\_@\}}. 
...
\texttt{makedefeasible\{foo,\_@F,\_@\}}. 
...
\texttt{myargth.flr}, then it can be declared in a \FLSYSTEM module
using
%% 
\begin{verbatim}
  :- use_argumentation_theory{foo}.
...
predicate {\tt \_cancel}. The default argumentation theory in \FLSYSTEM  
...
type to type. However, certain methods are more or less common:
 ...
 \begin{itemize}
    \item {\tt toSymbol}, which applies to a data type constant and
      ...
      {\tt "12:44:23"$\hat{~}\hat{~}$\_time} then {\tt ?Y[toSymbol->'12:44:23']} will
      be true.
    \item {\tt toType}( parameters ), which applies to any class
      ...
      type. For instance, {\tt
        \_time[toType(12,23,45) -> "12:23:45"$\hat{~}\hat{~}$\_time]}.  The other will
      apply to the constant symbol representation of the data type. For instance,
      {\tt \_time[toType('12:23:45')->"12:23:45"$\hat{~}\hat{~}$\_time]}. 
...
\emph{strings} (class \texttt{\_string}) , \emph{IRIs} (class
\texttt{\_iri}), and \emph{abstract symbols} (class \texttt{\_symbol}). 

Basically, abstract symbols are all the Prolog atoms that do not
serve as internal representations for the data types
of the form \texttt{"..."$\hat{~}\hat{~}$\_iri} or
\texttt{"..."$\hat{~}\hat{~}$\_strings}.\footnote{
  %%
...
meaning as in files A and B. However, prefix \texttt{foo} was overwritten
...
meaning. Note that even though \texttt{moo} was overwritten by file B, this
...
the \FLSYSTEM module {\tt \_basetype}. They are described here by their
signatures.
%%
\paragraph{Class methods:}
%% 
\begin{itemize}
\item  {\tt \_iri[toType(\_symbol) => \_iri]}
\item {\tt \_iri[=> isTypeOf(\_object)]}
\end{itemize}
%% 
...
\item
  {\tt \_iri[path *=> \_symbol]}
\item
  {\tt \_iri[query *=> \_symbol]}
\item
  {\tt \_iri[fragment *=> \_symbol]}
\end{itemize}
%% 
...
\begin{itemize}
\item  {\tt \_iri[toSymbol *=> \_symbol]}  
\item  {\tt \_iri[*=> equals(\_object)]}   
\item  {\tt \_iri[typeName *=> \_symbol]}   
\end{itemize}
...
\item      {\tt ?- \_iri[toType('http://foo.bar.com/abc') ->}\\
  \hspace*{5cm}{\tt "http://foo.bar.com/abc"$\hat{~}\hat{~}$\_iri]@\_basetype}  
\item      {\tt ?- \_"http://foo.bar.com/abc"$\hat{~}\hat{~}$\_iri[host -> 'foo.bar.com']@\_basetype}  
\end{itemize}
...
\begin{itemize}
\item {\tt
    \_dateTime[toType(\_integer,\_integer,\_integer,\_integer,\_integer,\_integer,\_decimal,
...
\item {\tt \_dateTime[toType(\_symbol) => \_dateTime]}  
\item {\tt \_dateTime[=> isTypeOf(\_object)]}  \\
  Tells if object belongs to the primitive type {\tt \_dateTime}. 
\end{itemize}

\paragraph{Component methods:}
\begin{itemize}
      \item {\tt \_dateTime[dateSign *=> \_integer]}  
      \item {\tt \_dateTime[year *=> \_integer]}  
      \item {\tt \_dateTime[month *=> \_integer]}  
      \item {\tt \_dateTime[day *=> \_integer]}  
      \item {\tt \_dateTime[hour *=> \_integer]}  
      \item {\tt \_dateTime[minute *=> \_integer]}  
      \item {\tt \_dateTime[second *=> \_integer]}  
      \item {\tt \_dateTime[zoneSign *=> \_integer]}  
      \item {\tt \_dateTime[zoneHour *=> \_integer]}  
      \item {\tt \_dateTime[zoneMinute *=> \_integer]}  
    \end{itemize}

\paragraph{Other methods:}
\begin{itemize}
      \item {\tt \_dateTime[toSymbol *=> \_symbol]}  
      \item {\tt \_dateTime[*=> equals(\_object)]}  
      \item {\tt \_dateTime[*=> lessThan(\_object)]}  
      \item {\tt \_dateTime[typeName *=> \_symbol]}  
      \item {\tt \_dateTime[add(\_duration) *=> \_dateTime]}  
    \end{itemize}
...
\begin{itemize}
      ...
      \item {\tt ?- "2001-11-23"$\hat{~}\hat{~}$\_dateTime[day -> 23]@\_basetype}  
      \item {\tt ?- "2001-11-23"$\hat{~}\hat{~}$\_dateTime[toSymbol ->
        '2001-11-23T00:00:00+00:00']@\_basetype}  
      \item {\tt ?- "2001-11-23T18:33:44-02:30"$\hat{~}\hat{~}$\_dateTime[add("-P22Y2M10DT1H2M3S"$\hat{~}\hat{~}$\_duration)
...
\paragraph{Class methods:}
\begin{itemize}
    \item {\tt \_date[toType(\_integer,\_integer,\_integer,\_integer,\_integer,\_integer,\_integer) => \_date]}
      \\
    ...
    -1. 
    \item {\tt \_date[toType(\_symbol) => \_date]}
    \item {\tt \_date[=> isTypeOf(\_object)]}\\
      Tells if object belongs to the primitive type {\tt \_date}. 
\end{itemize}

\paragraph{Component methods:}
\begin{itemize}
\item {\tt \_date[dateSign *=> \_integer]}   
\item {\tt \_date[year *=> \_integer]}   
\item {\tt \_date[month *=> \_integer]}  
\item {\tt \_date[day *=> \_integer]}   
\item {\tt \_date[zoneSign *=> \_integer]}   
\item {\tt \_date[zoneHour *=> \_integer]}  
\item {\tt \_date[zoneMinute *=> \_integer]}    
\end{itemize}

\paragraph{Other methods:}
\begin{itemize}
\item {\tt \_date[toSymbol *=> \_symbol]}   
\item {\tt \_date[*=> equals(\_object)]}   
\item {\tt \_date[*=> lessThan(\_object)]}   
\item {\tt \_date[typeName *=> \_symbol]}   
\item {\tt \_date[add(\_duration) *=> \_date]}   
\end{itemize}
...
\begin{itemize}
  ...
  \item {\tt "-237-11-23"$\hat{~}\hat{~}$\_date}\\
    Note that this date refers to year 238 BCE.
  \item {\tt ?- "2001-11-23"$\hat{~}\hat{~}$\_date[day -> 23]@\_basetype}  
  \item {\tt ?- "2001-11-23"$\hat{~}\hat{~}$\_date[toSymbol ->
      '2001-11-23+00:00']@\_basetype}  
  \item {\tt ?- "2001-11-23-02:30"$\hat{~}\hat{~}$\_date[add("-P2Y2M10D"$\hat{~}\hat{~}$\_duration) ->}\\
  \hspace*{7cm}{\tt
...
\begin{itemize}
\item {\tt
    \_time[toType(\_integer,\_integer,\_decimal,\_integer,\_integer,\_integer)
    => \_time]}
  \\
  The arguments represent hour, minute, second, time zone sign, time
  zone hour, and time zone minute.
  \item {\tt \_time[toType(\_symbol) => \_time]}
\item {\tt \_time[=> isTypeOf(\_object)]}
  \\
...
\paragraph{Component methods:}
\begin{itemize}
\item {\tt \_time[hour *=> \_integer]}
\item {\tt \_time[minute *=> \_integer]}
\item {\tt \_time[second *=> \_integer]}
\item {\tt \_time[zoneSign *=> \_integer]}
\item {\tt \_time[zoneHour *=> \_integer]}
\item {\tt \_time[zoneMinute *=> \_integer] }
\end{itemize}

\paragraph{Other methods:}
\begin{itemize}
\item {\tt \_time[toSymbol *=> \_symbol]}
\item {\tt \_time[*=> equals(\_object)]}
\item {\tt \_time[*=> lessThan(\_object)]}
\item {\tt \_time[typeName *=> \_symbol]}
\item {\tt \_time[add(\_duration) *=> \_time] }
\end{itemize}
...
\item {\tt ?- \_time[toType(12,44,55) ->
    "12:44:55"$\hat{~}\hat{~}$\_time]@\_basetype}
\item {\tt ?- "12:44:55"$\hat{~}\hat{~}$\_time[minute -> 44]@\_basetype}
\item {\tt ?- "12:44:55"$\hat{~}\hat{~}$\_time[toSymbol -> '12:44:55']@\_basetype}
\item {\tt ?- "12:44:55"$\hat{~}\hat{~}$\_time[add("PT2M3S"$\hat{~}\hat{~}$\_duration) -> "12:46:58"$\hat{~}\hat{~}$\_time]@\_basetype }\\
...
\begin{itemize}
\item
  {\tt \_duration[toType(\_integer,\_integer,\_integer,\_integer,\_integer,}
    \hspace*{5cm}{\tt \_integer) => \_duration]}
  \\
  The meaning of the arguments (in that order) is: year, month, day, hour, minute, second.
\item
  {\tt \_duration[toType(\_symbol) => \_duration]}
\item
  {\tt \_duration[=> isTypeOf(\_object)]}
  \\
...
\paragraph{Component methods:}
\begin{itemize}
      \item {\tt \_duration[year *=> \_integer]}  
      \item {\tt \_duration[month *=> \_integer]}  
      \item {\tt \_duration[day *=> \_integer]}  
      \item {\tt \_duration[hour *=> \_integer]}
      \item {\tt \_duration[minute *=> \_integer]}  
      \item {\tt \_duration[second *=> \_integer]}   
    \end{itemize}

\paragraph{Other methods:}
\begin{itemize}
\item {\tt \_duration[toSymbol *=> \_symbol]}  
\item {\tt \_duration[*=> equals(\_object)]}  
\item {\tt \_duration[*=> lessThan(\_object)]}  
\item {\tt \_duration[typeName *=> \_symbol]}  
\item {\tt \_duration[add(\_duration) *=> \_duration]}  
\end{itemize}

\paragraph{Examples:}
\begin{itemize}
\item {\tt "P5Y5M10DT11H24M22S"$\hat{~}\hat{~}$\_duration} 
\item {\tt ?- "-P2Y05M10DT11H24M22S"$\hat{~}\hat{~}$\_duration[minute -> 24]@\_basetype}
\end{itemize}
...
%%
\begin{itemize}
    \item {\tt \_boolean[toSymbol => \_symbol]}  
    \item {\tt \_boolean[=> isTypeOf(\_object)]}  
\end{itemize}
...
\begin{itemize}
\item {\tt \_boolean[toSymbol *=> \_symbol]}  
\item {\tt \_boolean[*=> equals(\_object)]}  
\item {\tt \_boolean[*=> lessThan(\_object)]}\\  
  Note: {\tt \_false[lessThan(\_true)]}. 
\item {\tt \_boolean[typeName *=> \_symbol]}  
\item {\tt \_boolean[rawValue *=> \_symbol]}
  \\
...
{\tt ?- "true"$\hat{~}\hat{~}$\_boolean[rawValue->?X]@\_basetype}.
...
\FLSYSTEM treats floating point numbers as synonymous with {\tt \_double} and
...
\paragraph{Class methods:}
\begin{itemize}
\item {\tt \_double[toType(\_decimal) => \_double]}\\  
  Converts decimals to doubles. Error, if overflow.
 \item {\tt \_double[toType(\_long) => \_double]}\\  
  Converts long integers to doubles.
 \item {\tt \_double[toType(\_string) => \_double]}\\  
 {\tt \_double[toType(\_symbol) => \_double]}\\
  Converts strings and symbols into doubles, if the textual representation
  of these values is a number.
{\tt \item \_double[=> isTypeOf(\_object)]}  
\end{itemize}

\paragraph{Instance methods:}
\begin{itemize}
\item {\tt \_double[floor *=> \_integer]}
\item {\tt \_double[ceiling *=> \_integer]}
\item {\tt \_double[round *=> \_integer]}
\end{itemize}

\paragraph{Other methods:}
\begin{itemize}
\item {\tt \_double[toSymbol *=> \_symbol]}
\item {\tt \_double[*=> equals(\_object)]}
\item {\tt \_double[*=> lessThan(\_object)]}
\item {\tt \_double[typeName *=> \_symbol]}
\item {\tt \_double[rawValue *=> \_double]}
  \\
...
{\tt ?- "1.2"$\hat{~}\hat{~}$\_double[rawValue->?X]@\_basetype}.
...
\paragraph{Class methods:}
\begin{itemize}
    \item {\tt \_long[toType(\_symbol) => \_long]}   \\
    {\tt \_long[toType(\_symbol) => \_long]}   \\
    ...
    \item {\tt \_long[toType(\_integer) => \_long]}   \\
     Converts long integers to arbitrary big integers.
    \item {\tt \_long[=> isTypeOf(\_object)]}  
\end{itemize}

\paragraph{Other methods:}
\begin{itemize}
\item {\tt \_long[toSymbol *=> \_symbol]}  
\item {\tt \_long[*=> equals(\_object)]}  
\item {\tt \_long[*=> lessThan(\_object)]}  
\item {\tt \_long[typeName *=> \_symbol]}   
\item {\tt \_long[rawValue *=> \_long]}
  \\
...
the method {\tt rawValue}.  For instance, {\tt ?-
  "12"$\hat{~}\hat{~}$\_long[rawValue->?X]@\_basetype}.
...
At present, \FLSYSTEM does not implement the {\tt \_decimal} and the {\tt
...
%% 
\begin{itemize}
\item {\tt \_string[=> isTypeOf(\_object)]}  
\item {\tt \_string[toType(\_object) => \_string]}  
\end{itemize}
%% 

Note that the method {\tt toType} in class {\tt \_string} can be used to
serialize any term as a string. For instance,
%% 
\begin{alltt}
   \prompt _string[toType(abc(cde))->?val]@_basetype.
...
\paragraph{Instance  methods:}
\begin{itemize}
\item {\tt \_string[*=> contains(\_string)]}  
\item {\tt \_string[concat(\_string) *=> \_string]}  
\item {\tt \_string[reverse *=> \_string]}  
\item {\tt \_string[length *=> \_integer]}  
\item {\tt \_string[toUpper *=> \_string]}  
\item {\tt \_string[toLower *=> \_string]}  
\item {\tt \_string[*=> startsWith(\_string)]}  
\item {\tt \_string[*=> endsWith(\_string)]}  
\item {\tt \_string[substring(\_integer,\_integer) *=> \_string]}  
  \\
...
\paragraph{Other methods:}
\begin{itemize}
\item {\tt \_string[*=> equals(\_object)]}  
\item {\tt \_string[*=> lessThan(\_object)]}  
\item {\tt \_string[typeName *=> \_symbol]}  
\end{itemize}
...
the method {\tt \_rawValue}.  For instance, {\tt ?-
  "abc"$\hat{~}\hat{~}$\_string[rawValue->?X]@\_basetype}.
...
\paragraph{Class methods:}
\begin{itemize}
\item {\tt \_list[=> isTypeOf(\_object)]}  
\item {\tt \_list[toType(\_list) => \_list]}  
\end{itemize}

\paragraph{Other methods:}
\begin{itemize}
\item {\tt \_list[*=> contains(\_list)]} 
  \\
  Tells if a list object contains the method's argument as a sublist.
\item {\tt \_list[*=> member(\_object)]}  
  \\
  ...
  member of the list.
\item {\tt \_list[append(\_list) *=> \_list]}  \\
  Appends one list to another and returns the resulting list.
\item {\tt \_list[length *=> \_long]}  
  \\
  Computes the length of the list.
\item {\tt \_list[reverse *=> \_list]}  
\item {\tt \_list[sort *=> \_list]}  
\item {\tt \_list[*=> startsWith(\_list)]}  
\item {\tt \_list[*=> endsWith(\_list)]}  
\item {\tt \_list[*=> subset(\_list)]}  
  \\
...
\paragraph{Other methods:}
\begin{itemize}
\item {\tt \_list[toSymbol *=> \_symbol]}  
\item {\tt \_list[*=> equals(\_object)]}  
\item {\tt \_list[typeName *=> \_symbol]}  
\end{itemize}
...
\item {\tt \flabort} or {\tt \flabort(?Message)} ---
  ...
  {\tt abort(?Message)@\_sys}, as explained in
...
\begin{verbatim}
    ...
    ?- catch{?X:a, \FLUNDEFEXCEPTION(?,?)@_prolog, true}.
\end{verbatim}
%%
...
must issue the command {\tt \_logforest} at the \FLSYSTEM
...
\end{verbatim}
%% 
will direct \FLSYSTEM to dump the entire forest log into the file {\tt foobar.flr}.
...
\end{verbatim}
%% 
will dump the trace in the above \FLSYSTEM format into the file {\tt foobar.flr}
...
as reducing log file sizes. \FLSYSTEM provides {\tt \_logforest(HideOptions)} and 
...
{\tt \_logforest(File,HideOptions)} will not be mixed since \FLSYSTEM
...
{\tt \_logforestlow}; it directs \FLSYSTEM to display forest logs 
...
\FLSYSTEM also provides {\tt \_logforestlow(HideOptions)} and 
...
To control these features, \FLSYSTEM provides the following predicates.

\paragraph{Timeouts.} The following calls can be used to control various
types of timeouts:
%%
\index{\FLSETTIMEOUT}
\index{\FLCLEARTIMEOUT}
%% 
\begin{alltt}
    ?- \FLSETTIMEOUT(\emph{Time},fail)@_prolog(flrerrhandler).
    ?- \FLSETTIMEOUT(\emph{Time},abort)@_prolog(flrerrhandler).
    ?- \FLSETTIMEOUT(repeating(\emph{Time})@_prolog,\emph{Handler})@_prolog(flrerrhandler).
    ?- \FLCLEARTIMEOUT@_prolog(flrerrhandler).
\end{alltt}
%% 
...
\item \emph{Handler} must be either a Prolog predicate defined in some
  Prolog module or a \FLSYSTEM predicate declared using the \texttt{:- prolog}  
...
\begin{alltt}
   ...
   ?- \FLSETTIMEOUT(repeating(2)@_prolog,
...
control:
%%
\index{\FLSETSUBGOALDEPTH}
%% 
\begin{alltt}
    ?- \FLSETSUBGOALDEPTH(\emph{TermDepth},\emph{ListDepth},fail)@_prolog(flrerrhandler).
    ?- \FLSETSUBGOALDEPTH(\emph{TermDepth},\emph{ListDepth},abort)@_prolog(flrerrhandler).
\end{alltt}
%% 
The \texttt{\FLSETSUBGOALDEPTH} command controls the maximum depth
...
\index{\FLSETANSWERDEPTH}
%% 
\begin{alltt}
    ?- \FLSETANSWERDEPTH(\emph{TermDepth},\emph{ListDepth},fail)@_prolog(flrerrhandler).
    ?- \FLSETANSWERDEPTH(\emph{TermDepth},\emph{ListDepth},abort)@_prolog(flrerrhandler).
\end{alltt}
%% 
This command controls the depth of the returned answers. The meaning of the
arguments in the same as in the case of \texttt{\FLSETSUBGOALDEPTH}.
...
mode:
%%
\index{\FLSETUNIFICATIONMODE}
%% 
\begin{alltt}
    ?- \FLSETUNIFICATIONMODE(fast)@_prolog(flrerrhandler).
    ?- \FLSETUNIFICATIONMODE(correct)@_prolog(flrerrhandler).
\end{alltt}
%% 
...
{\tt compiler\_options/1}. It takes one argument --- a list of
...
the prompt:
\index{\_production(on)}
\index{\_production(off)}
%% 
\begin{verbatim}
   ?- _production(on).
\end{verbatim}
...
\begin{enumerate}
  ...
  \item  Use {\tt \_dump/1} to strip \FLSYSTEM macros from the output of the
    \FLSYSTEM compiler.
...
\FLSYSTEM prompt. It is also possible to put the subgoal {\tt \_trace} in
...
As with {\tt \_trace}, \FLSYSTEM provides a convenient shortcut that allows
...
Of course, \FLSYSTEM has much more: signatures, inheritable and
