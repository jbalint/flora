...
\item {\tt \bs{}op(Precedence,Associativity,Operator)}:
...
\begin{qrules}
{\tt :- \bs{}op(\emph{Precedence},\emph{Type},\emph{Name}).}
\end{qrules}
%%
For instance, 
%%
...
\FLSYSTEM has a number of reserved keywords, which cannot be used directly
\emph{as \underline{unary} predicate names}. However, they can be used as
0-ary, binary, etc., predicate names or as function symbols or any arity:
%% 
\begin{itemize}
\item  \texttt{while}
\item \texttt{loop}
...
\begin{quote}
 \tt
 :- import '\bs{}load'/1, '\bs{}compile'/1 from flora2.\\
 ?- '\bs{}load'({\it flora-file} >> {\it flora-module}).\\
 ?- '\bs{}load'({\it flora-file}).\\
 ?- '\bs{}compile'({\it flora-file} >> {\it flora-module}).\\
 ?- '\bs{}compile'({\it flora-file}).
\end{quote}
%%
...
\begin{quote}
 \tt
 :- import '\bs{}isloaded'/1 from flora2.\\
 ?- '\bs{}isloaded'({\it flora-module-name}).
\end{quote}
%%
...
{\tt (... =.. ...)@\bs{}prolog}.  This is rarely used, however. One might use
...
\end{verbatim}
%% 
However, this does not make logical sense. The fact \texttt{c[|neg att->1|]} says
...
\end{verbatim}
%% 
the explicit negative statement \texttt{cc[|neg attr1->abc|]} blocks the
inheritance of \texttt{c[|attr1->1|]} down to both \texttt{cc} and \texttt{obj}.
Likewise, the explicit negative information \verb|obj[neg attr2->cde]|
...
type to type. However, certain methods are more or less common:
 ...
 \begin{itemize}
    ...
    \item {\tt \bs{}equal}( constant ), which tells when the given datatype constant equals some other term.
...
\item {\tt \bs{}boolean[|rawValue => \bs{}symbol|]}
  ...
  the {\tt \bs{}boolean} data type. 
\end{itemize}

\paragraph{Caveat:}  The long form and the short form must really be the
same, i.e., {\tt "true"$\hat{~}\hat{~}$\bs{}boolean} and {\tt true}
must denote the same constant.  However, this has not been implemented yet.
To extract the short representation part from a \texttt{\bs{}boolean}   data
type one should use the method {\tt \bs{}rawValue}.  For instance,
...
\paragraph{Caveat:} The long form and the short form must really be the
same, i.e., {\tt "abc"$\hat{~}\hat{~}$\bs{}string} and {\tt abc} must denote the
same constant.  However, this has not been implemented yet.
To extract the atom part from a {\tt \bs{}long}  data type one should use
the method {\tt rawValue}.  For instance, {\tt ?-
...
%handleException(?Reason,?X) :-
        format('~w: ?X=~w~n',[?Reason,?X])@\prolog(format), fail.
\end{verbatim}
%%
...
when inheritance is overridden by
...
directive {\tt \bs{}op(400,xfx,fff)} in
...
\item[{\tt :-  \bs{}op({\it precedence},{\it type},{\it operator})}]
...
\index{compiler directive!{\tt op}}
 ~~~ \\
  ...
  {\tt fx}, {\tt xf}, {\tt xfy}, etc.
  Note that the {\tt \bs{}op} directive is confined to
  ...
  foo}.
\item[{\tt :-  \bs{}op({\it precedence},{\it type},[{\it operator}, ..., {\it operator}])}]
~~~  \\
  Same as above, except that this directive defines a list of operators
  with the same precedence and type.
\item[{\tt ?-  \bs{}op({\it precedence},{\it type},{\it operator})@\emph{module}}]
 ~~~ \\
