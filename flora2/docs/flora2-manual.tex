...
\FLORA has a \emph{simplified} installation procedure for the
...
release of \FLORA; it \emph{cannot} be used if you obtained the software
...
one of the latest versions of XSB that is compatible with the \FLORA release.
...
\end{alltt}
%% 
This will install both XSB and \FLORA, appropriately configured.
The script to run \FLORA is \texttt{\textbf{DIR}/flora2/runflora}.  
...
\FLORA will be installed with the appropriate shortcuts placed on the
...
\paragraph{Starting \FLORA.}
The command to run \FLORA is \texttt{./runflora} (Linux/Mac/Cygwin)
...
under Windows, some internal \FLORA files might get recompiled on their
...
\subsection{Installing \FLORA from SVN}

This procedure is significantly more complex than the one for installing
the official \FLORA releases described above. It is suitable only for the
users who need the very latest development version of \FLORA---either in
...
Next, check out \FLORA from its SVN repository at
\url{http://sourceforge.net/p/flora/src/HEAD/tree/trunk/flora2/} into a
separate directory \emph{outside} the XSB installation tree.  The \FLORA
...
Once Visual C++ Express is installed and \FLORA is checked out from SVN,
...
%% 
Save the changes. 
Then configure \FLORA as follows:
%%
\begin{alltt}
   cd flora2
   makeflora clean
...
for \FLORA in your SVN checkout; it should have the form
...
on your Windows
system to some directory accessible from your account. XSB and \FLORA
will place
...
\FLORA, you must use the \texttt{makeflora -c64} command instead of
...
the parts of \FLORA that are written in the C language.


\paragraph{A note on  \FLORA in Windows under Cygwin.}
...
applications of \FLORA require a lot of main memory.


\section{Running \FLORA}
...
\index{FLORA\_RC\_FILE environment variable}
\index{environment variable!FLORA\_RC\_FILE}
\paragraph{The initialization file.}
When \FLORA starts up, it first executes the commands found in the
initialization file, if it is specified and exists. The initialization file
is specified as a value of the \texttt{FLORA\_RC\_FILE} environment variable
...
The commands in the initialization file can be any kind of \FLORA queries
or commands. They must be specified exactly as they would be written in the
interactive \FLORA shell, i.e., without the query prefix ``\texttt{?-}''
and they must be terminated with the period.
For instance, if
\texttt{FLORA\_RC\_FILE} is set to \verb|~/test/florarc| (note: the
...
There one restrictions on the initialization file: a comment cannot be the
last statement.
...
\FLORA's \texttt{\_load}  and \texttt{\_add} commands try to be smart in
...
%%
\begin{enumerate}
\item \FLORA uses ``,'' to separate methods in F-logic frame formulas. The version of
...
%
In a \FLORA, any combination of conjunction, disjunction, and
...
information about the source code where the various \FLORA statements
...
\item  \texttt{\_@} -- the module into which the file containing this
  quasi-constant is loaded. In the \FLORA shell, this quasi-constant is set
  to be the atom \texttt{main}. 
\item \texttt{\_@F} -- the file in which this quasi-constant occurs. In the
  \FLORA shell, this quasi-constant is set to be the atom
  \texttt{'(interactive)'}. 
...
In addition, \FLORA provides a number of
\emph{quasi-variables}. Quasi-variables are similar to quasi-constants in
that \FLORA replaces them by constants. However, this happens at \emph{run
...
\end{verbatim}
%% 
whether this expression occurs in the head or the body of a rule. \FLORA
...
oids are not important, \FLORA provides the
...
compiler to generate a new Skolem symbol. To solve this problem, \FLORA allows 
...
rules.  The general rule is that the scope of a numbered Skolem is a \FLORA
...
flora2 ?- _isinteger(?X), ?X=1.

No

flora2 ?- _isinteger(?X,wish), ?X=1.

?X = 1

Yes

flora2 ?- _isinteger(?X,must), ?X=1.

?X = 1

Yes

flora2 ?- _isinteger(?X,must).

++Abort[FLORA]> in file (runtime) on line 1: instantiation error in builtin:
                _isinteger(_h3464); unbound argument

flora2 ?- _isinteger(?X,wish).
...
\FLORA supports import of entire modules through the {\tt importmodule}
...
%%
\FLORA supports three kinds of negation: a \emph{Prolog-style negation}
...
negation is that \FLORA lets one apply it to formulas that contain variables.
...
\FLORA takes a different approach. For body-only variables that appear
...
In that case, \FLORA \emph{defers} the call in the hope that \texttt{?X}
...
In both cases, \FLORA makes an open domain assumption by refusing to commit
...
In \FLORA one must then say this explicitly and in a natural way through
...
\index{\url{<~~}}
\index{\url{<~~>}}
Unlike most other rule engines, \FLORA supports a much larger variety of
...
Quantifiers in \FLORA are implemented through the well-known Lloyd-Topor
...
Lloyd-Topor transform was originally defined, \FLORA gives a
...
Because by default \FLORA tables everything that looks like a frame,
...
This kind of reasoning will not work in \FLORA because facts are stored in
an internal database in an unordered fashion and one cannot know when they
are going to be matched against the query. So, in \FLORA an open call
...
amount of main memory can be saved by telling \FLORA to not table
...
\index{rule Id}
Every rule in \FLORA is assigned a unique \emph{rule Id}.  A rule Id
...
name and the module components of rule Ids, \FLORA also provides the
...
become known in the \FLORA shell. However, if an \emph{added} file has an
...
%% 
\begin{verbatim}
   flora2 ?- _string[toType(abc(cde))->?val]@_basetype.
...
As with nested frame literals, the \FLORA compiler assumes that path
