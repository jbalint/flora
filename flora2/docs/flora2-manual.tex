...
\item
  ...
  \index{frame!empty}
  \index{empty frame}
\item \emph{Empty frames}
  Normally, a frame has one or more statements about the frame's object,
  which appear inside the brackets, as in
  %% 
  \begin{quote}
   \texttt{John[spouse\mvd Mary, age\mvd 31]}
  \end{quote}
  %% 
  However, it is also possible for a frame to be \emph{empty}, like this
  (an empty object specification):
  %% 
  \begin{quote}
   \texttt{Mary[]}  
  \end{quote}
  %% 
  or like this (and empty class signature):
  %% 
  \begin{quote}
   \texttt{Mary[| |]}  
  \end{quote}
  ...
  (e.g., \texttt{Mary} in our case) is \emph{known to exist}. This means that
...
\item \emph{Rules} are constructs of the form $head \texttt{:-} 
  ...
  using the semicolon symbol ``;''. As usual in logic languages, a single rule 
...
evaluated. Otherwise, an error is issued. If you want to do equation and
constraint solving, \texttt{\bs{}is/2} is a wrong predicate to do so: see
Section~\ref{sec-clp} for the proper way to do this.
Also, it should be noted that in the \texttt{\bs{}is/2} expressions
...
a predicate and also to load the program file defining it into a given
\FLSYSTEM user module. The second syntax is used when the \FLSYSTEM program
